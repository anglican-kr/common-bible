...
- **소급 기록** — GitHub 에만 있던 머지 PR 본문 308건(2025-08-17 ~ 2026-09-20)을 `…-prNNN-<slug>.md` 로 저장(날짜 = KST 머지일, 내용 무수정, 하단 attribution 줄만 제거).
...
- 과거 공백(worklog 분할 + PR 본문 소급 저장)까지 메운다.
...
- `ledger.yml` 은 로컬 실행 불가 — 이 PR 의 CI 로 확인. 첫 실행은 소급 기록 파일까지 검사해 실패 → "추가된 원장 중 하나 이상이 형식을 만족하면 통과" 로 완화 후 통과. 브랜치 보호 필수 검사에 `Change ledger` 추가 완료(`Unit tests` + `Change ledger`).
