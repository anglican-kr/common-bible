...
`index.html`이 18개 `<script>` 태그를 dependency order로 **명시적으로 나열**한다 (ADR-019 §"채택 방식"). ESM이 자동으로 `import` 그래프를 해소하는 형태는 **아니다** — `<script type="module">`도 결국 브라우저가 각 태그를 발견 순서로 로드한다.
...
모듈 간 의존은 여전히 `window.X` facade(`window.driveSync` / `window.appHelpers` 등)가 주류이고, ADR-019는 facade를 **점진 폐기** 대상으로 정의 — 신규 코드는 `import`/`export`로 작성 권장.
...
위 19개 파일 외에 `js/types.d.ts`는 TS 컴파일러 입력 전용(`@typedef`/`@type`이 참조). 의존 순서·로드 모드 변경 시 ADR-019 참조.

### 4.1 라우팅 (History API SPA)

`js/app/views-routing.js`가 `pushState`/`popstate` 기반으로 라우팅하고, `sw.js`는 모든 navigation 요청을 `/index.html`로 폴백한다. 단, `/privacy.html` 같은 stand-alone 페이지는 그대로 통과시킨다 ([ADR-009](decisions/009-history-api-routing.md), `sw.js:113`).
