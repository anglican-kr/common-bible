...
`release.py`는 `version.json`의 `version`과 `sw.js`의 `SHELL_CACHE` 값을 한 트랜잭션처럼 함께 올린다 — 둘이 어긋나면 SW가 새 셸을 가져오지 못한다. `DATA_CACHE`/`AUDIO_CACHE`는 포맷·인코딩이 실제로 바뀐 릴리스에서만 `--bump-data`/`--bump-audio`로 명시적으로 올린다. 빌드 zip은 git ignore된 산출물(`data/bible/*.json`, `data/search-*.json`)을 포함해 호스트에 그대로 풀 수 있는 형태다.
