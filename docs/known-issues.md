...
[`js/app/views.js:1010`](../js/app/views.js#L1010) 과 [`js/app/citations.js:677`](../js/app/citations.js#L677) 이 `line[0] === '"'` 로 운문 행의 내어쓰기(hanging punctuation, ADR-006) 클래스를 판정한다. 곱슬 따옴표는 못 알아본다.

- **ADR-039 시편 렌더링이 붙는 시점**: 전례시편 본문은 이미 `“` 로 시작하므로 그때 바로 이 판정이 실패한다.
- **`source/` 전환 시**: 데이터만 바꾸면 운문 첫 행 따옴표 내어쓰기가 전부 사라진다. 반드시 같은 PR에서 고칠 것.

`“`·`‘` 도 받도록 넓히면 된다. [`js/app/views.js:925`](../js/app/views.js#L925) 의 `isPunctOnly` 정규식은 이미 곱슬 따옴표를 포함하고 있어 참고가 된다.
...
**(e) 파급 범위** — 73권 JSON + 검색 인덱스 4종이 재생성되고 `tests/test_snapshots.py` 픽스처가 통째로 갱신된다. ADR-006 §Hanging punctuation 의 문구도 함께 고쳐야 한다. `source/` 만 커밋하고 산출물은 `build.yml` 이 자동 커밋백하도록 둘 것 — 산출물을 직접 커밋하면 빌드가 no-op 이 되어 앱 저장소 자동 동기화가 끊긴다.
