...
| 2026-05-10 | Phase 7a 작성 완료 | `js/app/views-routing.js` 신설(~540줄). 영역: Data fetching(`loadBooks`/`loadVersion`/`loadChapter`/`loadPrologue` + `booksCache`/`appVersion` 모듈 상태), Rendering helpers(`setTitle`/`setBreadcrumb`/`setTitleWithDivisionPicker`/`setTitleWithChapterPicker`/`buildDivisionBreadcrumb`/`divisionLabels`/`divisionOrder`/`effectiveDivision` + `DIVISION_LABELS`/`DIVISION_ORDER`/`OT_SUBCATEGORY`/`OT_SUBCATEGORY_ORDER`/`OT_SUBCATEGORY_LABELS`/`VULGATE_DIVISION_LABELS`/`VULGATE_DIVISION_ORDER` 상수), Pull-to-refresh IIFE(모바일 페이지 레벨 새로고침 제스처), `initCompactHeader`(스크롤 60px 초과 시 breadcrumb 접기). 세 영역이 app.js 곳곳에 흩어져 있어 sed 추출 3회 후 합류. 모듈 분할 1~6에 비해 훨씬 적은 라인이지만(낮은 결합도 영역만 선택) 현 PR을 7b의 큰 규모와 분리하는 게 머지/검증 부담 측면에서 유리. **window facade 13건**: `loadBooks`/`loadVersion`/`loadChapter`/`loadPrologue`/`setTitle`/`setBreadcrumb`/`setTitleWithDivisionPicker`/`setTitleWithChapterPicker`/`buildDivisionBreadcrumb`/`divisionLabels`/`divisionOrder`/`effectiveDivision`/`initCompactHeader` (+ `getBooksCache` 이전 + `appViewsRouting` aggregate). app.js Phase 7b 영역에서 직접 참조하던 `DIVISION_LABELS`/`OT_SUBCATEGORY`/`OT_SUBCATEGORY_ORDER`/`OT_SUBCATEGORY_LABELS` 4개 상수도 window facade로 노출 — 이름이 대문자 const라 일관성 약간 깨지지만 일시적(7b에서 caller가 같은 모듈로 들어오면 자연 해소). app.js facade 정리: `window.setTitle`/`window.setBreadcrumb`/`window.getBooksCache` 항목 이전. types.d.ts에 글로벌 declare 11건 + `Window.appViewsRouting`/`DIVISION_LABELS`/`OT_SUBCATEGORY`(외 3) 추가. SHELL_CACHE shell-60 → shell-61. app.js 2,126 → 1,661줄 (−465). main + worker tsc 0 error, app.config 5 (기존 gtag/dataLayer), 유닛 217 통과 |
