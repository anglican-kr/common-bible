...
| PR-5 | 검색 + 검색 시트 + 검색 히스토리 패널                                                                        | L3245-L4229 (PR-4 머지 후 라인) | 작성 완료 (커밋 대기) | —       |
...
| 2026-05-09 | PR-3 머지 (#83) | CI Unit tests + Cursor Bugbot 모두 green, main 통합 (rebase) |
| 2026-05-09 | PR-4 작성 완료 | `js/app.js` L2702-L3233 JSDoc 보강 — `parsePath`의 `parseInt(query.get("page") ?? "", 10)` null 가드, `updatePageMeta` 빈 객체 destructure 회피(JSDoc), `route()`의 view branch마다 bookId/chapter narrow 가드, 전역 anchor click 핸들러에 `e.target instanceof Element` 가드 + `closest("a[href]")` `HTMLAnchorElement` cast, `progress.max` Number cast, `_updateFabLift` 호출 시 `querySelector(".chapter-nav")` `HTMLElement` cast. 모듈 헤드의 `$searchInput` `HTMLInputElement` narrow, `saveReadingPosition` signature `chapter: number \| "prologue"` 확장(ReadingPosition 일관). baseline 280 → 잔여 262 (PR-4 영역 0). main + worker tsc 0 error, 유닛 111건 통과 |
| 2026-05-09 | PR-4 머지 (#84) | CI Unit tests + Cursor Bugbot 모두 green, main 통합 (rebase) |
| 2026-05-09 | PR-5 작성 완료 | `js/app.js` L3245-L4229 JSDoc 보강 — 모듈 헤드 `$searchSheetInput` `HTMLInputElement` narrow, `getElementById("app-header")` optional chaining, `createSearchHistoryController.close({restoreFocus})` 빈 객체 destructure 회피, `dataset.scrollY = String(scrollY)`, chip click 핸들러 `e.target instanceof Element` 가드. `initSheetDrag`/`initBookmarkSheetDrag`/`initBookmarkDrawerResize`/`initCompactHeader`의 `getElementById(...)` 호출을 `_$` 헬퍼로 통일 — closure 안 inner 함수에서 narrow 유지(early return + nullable const는 closure narrow 유실). baseline 262 → 잔여 223 (PR-5 영역 0). main + worker tsc 0 error, 유닛 111건 통과 |
