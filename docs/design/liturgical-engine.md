...
후보를 모으는 곳 **다섯**: ① 격자 좌표(temporal) ② `date` 매치(sanctoral) ③ `lunar` 매치(KASI) ④ 규칙 파생(temporal-feasts 의 `rule` 평가) ⑤ **본문 쪽 날짜 인덱스**(readings·collects 의 `date`/`lunar`).

⑤ 가 없으면 **성인력 표에 행이 없는 날짜 본문 16건이 통째로 빠진다**(§5.4 의 outer join — 「성탄주간」 계열). 정의 표만 훑으면 그 날은 「아무 날도 아님」으로 나오고 본문이 있는데도 못 찾는다. 후보로 올린 뒤에는 선택된 관측일의 `date`·`lunar`·`name` 을 `findReadings`·`findCollects` 에 그대로 넘겨야 한다 — 좌표만 넘기면 같은 조회를 두 번 하게 된다.
