...
| **`precedence: 7` 이 두 품계를 공유한다** | `minor_feast`(96) 와 `commemoration`(16) 이 같은 7. **실제 겹침이 7건 있다**(2026-09-05 실측: 02.14 · 03.01 · 05.03 · 06.09 · 08.05 · 09.04 · 12.31) | 동률 tie-break 이 규정에 없다. 둘 다 sanctoral 고정일이라 §6.2 ④ 범주 사다리로도 갈리지 않으므로 `id` 사전순으로 결정성만 확보하고 **규정 확인 대상**으로 둔다(§9 미결6) |
...
| **부활밤 독서에 가해(A)가 없다** | 실측(2026-09-13): `부활밤` 독서 레코드는 **B·C 둘뿐**이고 복음이 주기를 탄다(B 마르 16:1-8 · C 루가 24:1-12). 본기도는 `year: null` 공통 1건이라 문제없다. 부활대축일은 A/B/C × 세트 2 = 6건으로 온전하다. 부활밤은 **감사성찬례가 맞다** — 복음이 있다(미결4 가 12.31 전야를 성무일과로 판정한 그 판별법) | 가해에 `findReadings(부활밤)` 이 **빈 결과**다. §2 원칙대로 throw 하지 않고 빈 배열로 넘긴다. **원료 누락으로 확정**(사용자 확인 2026-09-13) — 가해 독서는 **로마 6:3-11 · 시편 114 · 마태 28:1-10** 이다. `first`·`psalm` 이 세 해 공통이고 복음만 주기를 타는 패턴에 정확히 맞는다(A 마태 · B 마르 · C 루가). ADR-037 미결5 의 격자 빈 칸 4칸을 `articles-supplement.json` 손입력으로 메운 것과 같은 종류라 **데이터 저장소 후속 과제**다(미결21) |
...
**반환은 스팬마다 `[from, to]` 구간의 배열이고, 구간은 그 달력년으로 잘려 있다**(9차 리뷰). 부활절 기준 스팬은 언제나 한 해 안에 들지만(부활은 3.22~4.25) **성탄 기간 둘은 해를 넘긴다** — 한 해에 두 조각으로 나타난다. 소비자는 범위를 직접 비교하지 않고 `inSpan(dateStr, name)`(구간 중 하나에 들면 참)으로 묻는다.
...
| **`christmasToBaptism`** | **① `Y`-01-01 ~ 주의 세례 주일(`Y`)** · **② `Y`-12-25 ~ `Y`-12-31** | **2** | §6.3 ① 소재일 제외 |
...
조각 ① 의 끝인 **주의 세례 주일은 `Y` 의 앵커만으로 나온다**(`first_sunday_after` 1.6 — 부활절과 무관). computus 횟수는 늘지 않는다(§4.9 최대 세 번 그대로).
...
| `status` | 뜻 | 누가 만드나 |
|---|---|---|
| `proper` | 그 날짜의 고유 후보(아래 ①~④)의 **초기 상태** — PR 2 에서는 전부 이 값이다. B1 은 승자가 아닌 고유 후보를 규칙에 따라 재분류한다: 이동하면 `transferred_out`, `commemorate_only`·C 그룹 「옮기기 어려우면」이면 `commemorated`, prec 7 이면 `omitted`. **재분류 뒤에도 `proper` 로 남는 것**은 ① 승자(`official`) ② 격자 주일·평일 후보(연중 평일 독서 — 어느 규칙도 옮기거나 생략하지 않으므로 「밀린 독서 보존」(ADR-036 §7)이 이 상태로 실현된다) ③ 고정일끼리의 동시 봉헌에서 진 쪽(9.29 설립 기념일 — §6.5, 미결18) ④ `transferable: false` 인 주요축일·대축일이 진 경우(현재 표에서 12.25 같은 대축일은 지지 않으므로 사례 없음 — 생기면 데이터 결함 신호) ⑤ 이동 상한 `MAX_SHIFT` 를 넘겨 옮기지 못한 것(§6.5 — `defects` 에 id 가 함께 실린다. 현재 표에서 사례 없음) | PR 2 (초기) · PR 3 (재분류) |
...
| `omitted` | 밀려서 그해 생략되는 prec 7 축일·기념일(§6.2 절차 ⑤, 기도서 「축일 … 겹치면 생략한다」). 12.29 토마스 베켓이 어린이들 도착에 밀리는 경우. `displacedBy` 를 가진다. 후보에는 남는다 — 「밀린 독서 보존」 | PR 3 (B1) |

`from`·`to` 는 **계보가 아니라 기원과 목적지**다 — 한 축일은 많아야 한 번 옮기므로(§6.2 ②) 「직전 자리」와 「원래 날짜」가 같다.

**핵심 계약(ADR-036 §7)**: `precedence` 는 그날의 **공식 축일 하나**를 정하지만, 엔진은 **밀려난 축일 · 기념만 하는 축일 · 연중 평일 독서를 버리지 않는다.** **승자는 엔진이 정해 `official` 로 낸다**(B1, §6.1) — 뷰가 우선순위를 다시 계산하지 않는다. 뷰의 선택은 「`official` 만 보일지, `candidates` 전부를 보일지」다. 목적은 사용자가 "축일에 밀린 기본 독서"를 놓치지 않게 하는 것 — 뷰 설정의 기본은 교회력에 맞는 독서만(`official`), 고급은 그날 읽을 수 있는 모든 독서(`candidates`)다.
...
실측 동률은 **둘**이다(2026-09-13, `temporal-feasts.json` 24행 대조): **성지주일 · 대림1주일**(temporal prec 2 ∧ 격자 절기 주일 2)과 **사계재 4행**(temporal 8 ∧ 격자 feria 8). 사다리로 temporal 이 이기므로 `official` 은 성지주일(홍) · 대림1주일 · 하계재(자 · `penitential`)이고 격자 후보는 `proper` 로 남는다 — 격자가 이기면 **전례색이 자 · 녹으로 뒤집힌다**. 나머지 셋은 동률이 아니라 precedence 로 갈린다(주의 변모 주일 `null` · 왕이신 그리스도 3 vs 격자 연중 주일 5).

**예외 하나 — 주님의 세례는 §4.5 와 어긋난다.** 데이터 행이 `ordinary/1/sunday` · prec 3 인데 §4.5 의 절기 스팬은 그날을 `christmas` 에 둔다. 격자를 스팬대로 성탄절기 주일(절기 주일 **2**)로 합성하면 prec 3 인 temporal 행을 **이겨** 「성탄 N주일」이 `official` 이 되고, 데이터 행대로 `ordinary/1`(연중 주일 5)로 합성하면 주님의 세례가 이긴다. 즉 **미결10 은 표현을 고르는 문제가 아니라 그날의 승자를 정하는 문제**다 — 사다리로는 풀리지 않는다(동률이 아니다). PR 1 이 절기 스팬을 짜기 전에 닫아야 한다(§9 미결10).
...
1. ◐ **동점 정렬 키** — **절반 닫혔다(2026-09-13)**: §6.2 ④ 를 **범주 사다리**(`도착 > temporal > sanctoral > 격자 > guard`)로 바꿔, 격자 ∧ temporal 동률(성지주일 · 대림1주일 · 사계재 4행)에서 승자와 전례색이 `id` 우연으로 갈리던 것을 없앴다. **남은 것은 같은 범주 안의 동률**이라 사다리가 가르지 못한다 — 12.25 세 행(미결12 와 같은 날짜) · prec 7 쌍 7건(미결6). 이 잔여는 데이터 결함 신호가 아니라 **규정이 순서를 정하지 않은 자리**이므로 사제 확인으로 닫고, 그때까지 `id` 사전순으로 결정성만 지킨다. 함께 남는 것: 진짜 결함성 동점을 개발 빌드에서 어떻게 드러낼지 — 리포트 경로가 데이터 저장소에만 있어 앱에서 알릴 방법이 미정이다(§6.5 `defects` 가 같은 문제의 첫 사례).
...
10. 🔴 **주님의 세례가 좌표를 둘 요구한다 — 그리고 그 선택이 승자를 바꾼다** — 이 날은 성탄 절기 안이라 연중 주차 조회를 건너뛰는데, 동시에 **연중1주일** 좌표도 내야 한다(정본 전사: 「성탄절기: 성탄일부터 주의 세례 주일(연중1주일)까지」). `resolveDate` 는 `coord` **하나**로 규정돼 있어 문서대로 따르면 둘 중 하나가 빠지고 그날의 후보·독서 경로가 통째로 사라진다.
...
20. **성 토요일 하루에 전례가 둘이다 — 시간 구분이 없다** — `easter_offset: -1` 에 `t-성-토요일`(prec `null` · 홍 · `lent`)과 `t-부활밤`(prec 1 · 백 · `easter`)이 함께 걸린다(§4.3). 품계로는 부활밤이 승자이고 ADR-036 §8 의 백색 기간도 「부활밤~」인데, 같은 절이 「성주간 전체 홍 — 성 토요일까지」도 말한다. **두 규정은 시간으로만 갈린다**(낮 홍 / 밤 백). 엔진은 날짜당 색 하나·승자 하나라 지금은 표현할 자리가 없다. 잠정은 **승자 = 부활밤 · 색 = 홍**(가드 유지, §6.4)이고, 부활밤의 백색은 후보의 `color` 로 남는다. 결정할 것: ① 뷰가 「낮/밤」을 가르는가(ADR-038) ② 가른다면 `ResolvedDate` 가 색을 둘 내는가 ③ 아니면 그날만 특례로 백을 내는가. 성무일과가 범위 밖(§1.2)인 것과는 다른 문제다 — 부활밤은 **감사성찬례**다. **사제 확인 대상**(검토 문서 Q10).
21. **부활밤 가해(A) 독서가 데이터에 없다 — 내용은 확정됐다** — 실측(2026-09-13) `부활밤` 독서 레코드는 B·C 둘뿐이다. 본기도는 `year: null` 공통 1건이라 빠짐이 없고, 부활대축일은 A/B/C × 세트 2 로 온전하다. **가해 독서는 로마 6:3-11 · 시편 114 · 마태 28:1-10**(사용자 확인 2026-09-13) — `first`·`psalm` 이 세 해 공통이고 복음만 갈리는 실측 패턴(B 마르 16:1-8 · C 루가 24:1-12)에 그대로 맞는다. **엔진이 할 일은 없다**: 없는 동안에는 §2 원칙대로 빈 결과로 넘기고 throw 하지 않는다. **데이터 저장소 몫**이다 — ADR-037 미결5 가 격자 빈 칸 4칸을 `articles-supplement.json` 손입력으로 메운 그 경로다. 메우기 전까지 가해(2026·2029·2032…)의 부활밤은 독서가 비고, 그 해에 이 날은 `official` 이므로 뷰에 그대로 드러난다.
