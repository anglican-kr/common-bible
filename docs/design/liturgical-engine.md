...
// TransferPass = { arrivals: Map<"YYYY-MM-DD", Candidate[]>, departures: Map<"YYYY-MM-DD", Candidate[]> }
```

캘린더 월간 뷰가 42칸을 그리며 `resolveDate` 를 42번 불러도 computus 는 최대 두 번(달 경계에서 두 해가 걸릴 때) 돈다. 캐시는 순수 파생값이라 무효화가 필요 없다.

`transfers` 는 §6.5 이동 패스의 결과다 — 그해 안에서 옮겨간 축일이 **어디에 도착했고 어디서 떠났는지**의 색인. PR 2 까지는 빈 패스이고 PR 3 이 채운다(§8). `resolveDate(d)` 는 `year(d)` 와 **`year(d) − 1`** 두 해의 캐시를 확보한다 — 12월 말에서 밀린 축일이 1월 초에 닿을 수 있기 때문이다(§6.5 연도 경계). 캐시라 한 해당 한 번이다.
 ...
 *   displacedBy?: string,   // 밀어낸 관측일의 id (in·out 양쪽)
 * }} Candidate */
```
...
| `transferred_out` | 이 날짜의 고유 후보였으나 밀려서 **다른 날로 간** 축일. `to`·`displacedBy` 를 가진다. 승자가 될 수 없고 뷰는 「→ 12.29」로 안내한다 | PR 3 (§6.5) |
| `optional` | 「옮겨 지킬 수 있다」는 선택 주일 봉헌(§6.2)의 목적지 주일에 얹히는 후보. 설정이 켜기 전에는 승자 판정에 참여하지 않는다 | PR 3 (§6.2) |
...
후보를 모으는 곳 다섯: ① 격자 좌표(temporal) ② `date` 매치(sanctoral) ③ `lunar` 매치(KASI) ④ 규칙 파생(temporal-feasts 의 `rule` 평가) ⑤ **도착 이동** — `yearCache.get(year).transfers.arrivals.get(date)`(§6.5). ①~④ 가 그 날짜의 **고유** 후보(`status: "proper"`)이고 ⑤ 만 다른 날짜에서 온다. 같은 패스의 `departures.get(date)` 에 있는 고유 후보는 `status: "transferred_out"` 으로 바꿔 낸다.
...
function computeTransfers(year, seed /* Y−1 에서 넘어온 arrivals */) {
  ...
  for (const d of daysOf(year)) {          // 1.1 → 12.31, 날짜 순 — 순서가 결과를 정한다
    ...
    const ranked  = rankByPrecedence([...proper, ...arrived]);   // §6.1 + §6.2 tie-break
    for (const c of ranked.slice(1)) {     // 승자를 뺀 나머지
      ...
      push(arrivals,   dest, { ...c, status: "transferred_in",  from: d,  displacedBy: ranked[0].id });
      push(departures, d,    { ...c, status: "transferred_out", to: dest, displacedBy: ranked[0].id });
    }
  }
...
**`isFree(dest, c)`** — `c` 가 그 날짜에서 **이길 수 있으면** 빈 날이다. 구체적으로 ① 고유 후보 중 `c` 보다 **높은**(유효값이 작은) 것이 없고 — 같은 값의 고유 후보는 도착 우선(§6.2 ①)으로 `c` 가 밀어내므로 막지 않는다 — ② 이미 도착해 있는 후보 중 `c` 와 같거나 높은 것이 없다(도착끼리는 먼저 온 쪽이 남는다). `precedence: null` 후보는 최하위라 막지 않는다. 여기에 **보호 기간 규칙**을 더한다 — **성지주일부터 부활2주일까지**(easter−7 ~ +7, 기도서 「성주간, 부활주간 8일」 — 주요축일 나-2) 안의 날은 격자상 `feria` 이거나 `rank: null`(성 토요일)이라도 prec 1 로 차 있는 것으로 본다(§6.3 ②). 이 규칙이 없으면 2027년 수태고지가 성 토요일에, 2035년에는 부활 월요일에 앉는다. 도착한 축일이 그 날짜의 prec 7 후보를 생략시키는 것(절차 ⑤)은 B1 의 `official` 판정에서 다룬다 — 패스는 좌석만 정한다.
...
**연도 경계.** `next_day` 는 해를 넘길 수 있다 — 12월 말에 밀린 축일이 1월 초에 닿는 경우. 현재 데이터로는 일어나지 않지만(12.26~28 은 늦어도 12.29 에 앉고, 12.31 실베스터·위클리프는 prec 7 이라 옮기지 않는다), 표가 바뀌면 조용히 깨질 자리라 규칙을 못박는다. `yearTransfers(Y)` = `computeTransfers(Y, spill(Y−1))`, 여기서 `spill(Y−1)` 은 **`computeTransfers(Y−1, ∅)`** 의 arrivals 중 날짜가 `Y` 인 것이다. `Y−1` 을 `Y−2` 의 spill 없이 계산해도 되는 이유: spill 은 1월 첫 며칠에만 닿을 수 있어(연쇄 상한 — 아래) `Y−1` 의 12월 결과에 영향을 줄 수 없다. 따라서 재귀는 깊이 1 에서 끊기고 computus 는 두 번이다. `resolveDate(d)` 가 `year(d)−1` 캐시까지 확보하는(§4.9) 것이 이 때문이다. 이 규칙은 **선택 봉헌 목적지에는 적용하지 않는다** — 그것들은 원래 날짜의 ±7일 안이고 해를 넘기는 날짜(1.6·2.2·11.1·승천)가 없다.

**연쇄 상한.** 루프는 `dest − d > 30` 이면 데이터 오류로 throw 한다 — 이론상 무한 루프는 없지만(연중 평일은 언제나 비어 있다) 상한이 없으면 표 결함이 조용히 먼 날짜로 번진다. 실제 최장 이동은 수태고지의 부활대축일 겹침(2035·2046)으로 **8일**(3.25 → 4.2)이고, 마르코의 부활대축일 겹침(2038)도 8일(4.25 → 5.3)이다. 성탄 3축일은 각각 하루씩이다. 유닛이 1900~2100 전 연도에서 최장 연쇄 ≤ 14 를 단언해 spill 논증(위)의 전제를 지킨다.

**선택 봉헌**(§6.2)은 이 루프 밖에서 같은 패스에 얹는다 — 원래 날짜의 고유 후보를 옮기지 않고 목적지 주일의 `arrivals` 에 `status: "optional"` 을 **더한다**. `isFree` 는 `optional` 을 무시한다(설정이 켜기 전에는 좌석을 차지하지 않는다).
