...
**`guard(d)`** — 보호 기간의 가상 점유자. **성지주일부터 부활2주일까지**(easter−7 ~ +7, 기도서 「성주간, 부활주간 8일」 — 주요축일 나-2) 안의 날에는 유효 precedence 1 · `transferable: false` 인 합성 후보 하나를 낸다. 그 `observance` 는 표에 없는 합성 객체로 **`id: "guard:easter-octave"` · `name: "성주간·부활 8일"`** 을 가진다 — 보호 기간에 밀린 축일(2035 요셉 · 2025 마르코)의 `displacedBy` 가 이 id 다. `guard:` 접두사는 정의 표의 id 에 나타나지 않는 예약 접두사이고, 뷰는 이 접두사를 보면 표를 찾지 않고 그 이름을 그대로 쓴다(「성주간·부활 8일에 밀려 4.28 로」). 보호 기간의 실제 승자(성 목요일·부활대축일 같은 temporal 행이나 격자 주일)가 따로 있으면 `rankByPrecedence` 가 그것을 `ranked[0]` 에 두므로 `displacedBy` 는 그 실제 id 가 되고, `guard` 가 `ranked[0]` 이 되는 것은 **그날 고유 후보가 feria 뿐인 성주간·부활주간 평일**에서다. 그 밖의 날에는 빈 배열. 이 점유자는 **기원 날짜의 순위**에도, 아래 **목적지 검사**에도 같은 함수로 들어간다 — 한쪽에만 두면 반쪽이다. 목적지에만 두면 성주간 평일에 **처음 놓인** B 축일이 그날의 `feria`(prec 8)를 이겨 `ranked[0]` 이 되고 아예 옮겨지지 않는다(2035-03-19 성 요셉 · 2025-04-25 성 마르코가 그렇다). 성 토요일이 `rank: null` 인 것도 이 점유자가 덮는다. 뷰에는 내지 않는다 — `resolveDate` 의 후보 출처 ①~⑤ 에 없다.
