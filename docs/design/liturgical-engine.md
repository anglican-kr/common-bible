...
| 기간 | 스팬 | 소비자 |
|---|---|---|
| `holyWeek` | 성지주일 ~ 성 토요일 (E−7 ~ E−1) | §6.4 색 가드(전체 홍) |
| `easterOctave` | 부활대축일 ~ 부활2주일 (E+0 ~ E+7) | §6.4 색 가드 |
| `transferGuard` | **성지주일 ~ 부활2주일** (E−7 ~ E+7) | §6.5 `guard(d)` — 위 둘의 합 |
| `christmasToBaptism` | 12.25 ~ 주의 세례 주일 | §6.3 ① 소재일 제외 |
| `ascensionToPentecost` | 승천 ~ 성령강림 전날 (E+39 ~ E+48) | §5.3 부활6주 금·토 본기도 병합의 근거(데이터 `_note`) |
| `christmasOctave` | 12.25 ~ 1.1 | 배너 · 후속 |
| `dec17to24` | 12.17 ~ 12.24 | §5.4 의 날짜 전용 본문 범위 확인 |
...
**활성화 계약 — 켜면 원래 날짜에서 떠난다**(2026-09-05 리뷰). 설정이 어떤 축일의 선택 봉헌을 켜면 그 축일은 **원래 날짜에서 목적지 주일로 실제로 옮긴다** — 두 번 지키지 않는다. 구체적으로 `computeTransfers(year, seed, activated)` 가 `activated: Set<observanceId>` 를 받아, 켜진 축일은 ① 원래 날짜에서 `transferred_out`(`to`: 목적지 주일, `displacedBy` 없음 — 밀린 것이 아니라 택한 것) ② 목적지 주일에서 `transferred_in`(`from`: 원래 날짜)으로 **도착 색인에** 들어가 그 주일의 순위에 참여한다(승천·공현·모든 성인은 prec 1 이라 그 주일의 승자가 되고, 주의 봉헌은 `outranks_sunday` 로 연중 주일을 이긴다) ③ 원래 날짜에서는 충돌 판정에 오르지 않으므로 **충돌 이동은 일어나지 않는다** — 2049 주의 봉헌을 켜면 1.31 주일 하나만 있고 2.3 은 생기지 않는다 ④ `optionals` 에는 실리지 않는다(켜진 것은 이미 도착이다). 꺼진 축일은 종전대로 원래 날짜 `proper` + 목적지 `optional`. **선택 봉헌 도착이 좌석 경합에서 우선한다.** 충돌 이동은 주일에도 앉을 수 있다 — 연중 주일은 prec 5 라 A(3)·C(4) 를 막지 못하므로(§6.1), 토요일에 명절 등 가변 후보에 밀린 A·C 는 다음날 연중 주일에 도착한다(현재 표에서 선택 봉헌 목적지 주일과 실제로 겹치는 사례는 2025~2050 에 없지만 원리상 배제되지 않는다). 그런 주일을 켜진 선택 봉헌도 노리면 **선택 봉헌이 이긴다**: 0단계가 날짜 루프 전에 그 주일의 `arrivals` 에 먼저 앉으므로 §6.2 ② 선착순에 의해 뒤의 충돌 도착은 `isFree` ② 에서 막혀 월요일로 간다. 이것은 우연이 아니라 **의도된 우선순위**다 — 목회적으로 택한 이동이 규칙에 밀린 이동보다 앞선다(§7 ⑦ 합성 케이스로 못박는다). **캐시**: `transferCache` 는 `activated` 에 의존하므로 설정이 바뀌면 통째로 비운다(§4.9) — 조합별로 키를 두지 않는다, 바뀌는 일이 드물고 패스는 싸다. `optional` 후보의 유일한 용도는 「이 주일에 옮겨 지킬 수 있음」 안내와 설정 진입점이다. 어느 UI 가 `activated` 를 채우는가는 ADR-038 의 몫이다(§9 미결15).
...
**이동 상한.** 한 번의 이동이 `MAX_SHIFT`(30일)를 넘으면 그 축일은 **옮기지 않고 `proper` 로 남기며** `defects` 에 id 를 적는다 — throw 하지 않는다. 이 분기에 이르는 `c` 는 언제나 고유 후보다: 도착은 그날의 모든 고유 후보보다 높고(`isFree` ①) 다른 도착이 없는 날에만 앉으므로(②) 항상 `ranked[0]` 이고 `ranked.slice(1)` 에 들지 않는다 — §7 ⑧ 이 「도착은 그 날의 승자」로 단언하는 그 성질이다. §2 의 설계 원칙(엔진이 throw 하는 경우는 데이터 파일 부재·프리로드 누락 둘뿐, 결손은 빈 결과 + 계속 진행)을 지키는 것이고, 이론상 무한 루프는 없지만(연중 평일은 언제나 비어 있다) 상한이 없으면 표 결함이 조용히 먼 날짜로 번지기 때문에 둔다. `defects` 는 §9 미결1 이 말한 「개발 빌드에서 드러낼 데이터 결함 신호」의 첫 사례다 — 앱에서 어떻게 알릴지는 그 미결에 얹는다. 실측(2025~2050)으로 가장 긴 이동은 **수태고지 2029·2040 의 15일**(3.25 성지주일 → 4.9 — 2018 미국 성공회와 같다), 그다음 요셉 2035·2046 의 14일(3.19 성주간 월 → 4.2), 수태고지 2043 의 12일이다. 성탄 3축일은 각각 하루다. 한 축일은 한 번만 옮기므로(§6.2 ②) 누적 상한은 필요 없다. 유닛이 1900~2100 전 연도에서 이동 ≤ 15 를 단언한다(§7 ⑧).
