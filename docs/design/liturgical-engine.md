 ...
 *   from?: string,          // 이 날짜에 옮겨 왔으면 — 직전 날짜 "YYYY-MM-DD"
 *   to?: string,            // 이 날짜를 떠났으면 — 다음 자리 "YYYY-MM-DD"
 *   displacedBy?: string,   // 밀어낸 관측일의 id (Observance.id — 이 날짜에서 밀렸을 때)
 * }} Candidate */
// 패스 내부에서만 쓰는 누적 이동일 `shifted` 는 공유 타입에 넣지 않는다 — resolveDate 가 벗겨 낸다(§6.5).
```
...
| `transferred_out` | 이 날짜를 **떠난** 축일 — 고유 후보였다가 밀렸든, 옮겨 왔다가 더 높은 도착에 다시 밀렸든. `to`·`displacedBy` 를 가지고, 옮겨 왔던 것이면 `from` 도 가진다(2035 성 요셉: 3.19 → 4.2 → 4.3 이라 4.2 에서는 `from: 3-19, to: 4-3`). 승자가 될 수 없고 뷰는 「→ 4.3」로 안내한다. 한 날짜에 한 축일은 **한 번만** 나온다 — 도착·출발을 두 항목으로 내지 않는다 | PR 3 (§6.5) |
...
후보를 모으는 곳 다섯: ① 격자 좌표(temporal) ② `date` 매치(sanctoral) ③ `lunar` 매치(KASI) ④ 규칙 파생(temporal-feasts 의 `rule` 평가) ⑤ **도착 이동** — `transfers.arrivals.get(date)`(§6.5). ①~④ 가 그 날짜의 **고유** 후보(`status: "proper"`)이고 ⑤ 만 다른 날짜에서 온다. 같은 패스의 `departures.get(date)` 에 있는 것은 `status: "transferred_out"` 으로 낸다 — 고유 후보였으면 그것을 바꾸고, 도착이었으면 `arrivals` 쪽 항목을 대신한다(같은 축일이 `arrivals`·`departures` 양쪽에 있으면 **출발이 이긴다**). 그리고 `transfers.optionals.get(date)` 를 `status: "optional"` 로 덧붙인다.
...
2. **도착끼리는 먼저 온 쪽** — 같은 값의 도착이 둘이면 **기원 날짜가 이른 쪽**이 남고 뒤에 온 쪽은 다음날로 간다(패스가 날짜 순으로 돌기 때문에 「먼저 처리된 쪽」과 같다). 기원 날짜까지 같으면 `id` 사전순으로 먼저 간 쪽이 남는다. 2038 년 마르코(기원 4.25)와 필립보·야고보(기원 5.1)가 부활 8일을 건너 둘 다 5.3 을 노릴 때 마르코가 5.3, 필립보·야고보가 5.4 인 것은 **기원 순**이지 `id` 순이 아니다. `isFree` ② 가 이 규칙의 구현이다(§6.5).
...
const MAX_SHIFT   = 30;   // 한 번의 이동 상한(날). 실측 최장 14 — 2035·2046 성 요셉 3.19 → 4.2
const MAX_LINEAGE = 60;   // 재이동을 합친 누적 상한(날). 실측 최장 15 — 같은 요셉 3.19 → 4.2 → 4.3
...
function computeTransfers(year, seed /* Y−1 에서 넘어온 arrivals */) {
  ...
  for (const d of daysOf(year)) {          // 1.1 → 12.31, 날짜 순 — 순서가 결과를 정한다
    ...
    for (const c of ranked.slice(1)) {     // 승자를 뺀 나머지 (guard 는 movesOnConflict 가 거른다)
      ...
      const shifted = (c.shifted ?? 0) + daysBetween(d, dest);           // 계보 누적 (엔진 내부 필드, 뷰에 안 낸다)
      if (daysBetween(d, dest) > MAX_SHIFT || shifted > MAX_LINEAGE) {  // 상한 — 옮기지 않고 그 자리에 남긴다
        defects.push(c.observance.id); continue;
      }
      const by = ranked[0].observance.id;  // Candidate 는 래퍼 — id 는 observance 아래에 있다
      push(arrivals,   dest, { ...c, status: "transferred_in",  from: d,  to: undefined, displacedBy: by, shifted });
      push(departures, d,    { ...c, status: "transferred_out", to: dest, displacedBy: by });  // c.from 은 그대로 (도착이었다면)
    }
...
**`isFree(dest, c)`** — `c` 가 그 날짜에서 **이길 수 있으면** 빈 날이다. 구체적으로 ① 고유 후보와 `guard(dest)` 중 `c` 보다 **높은**(유효값이 작은) 것이 없고 — 같은 값의 고유 후보는 도착 우선(§6.2 ①)으로 `c` 가 밀어내므로 막지 않는다 — ② 이미 도착해 있는 후보 중 `c` 와 같거나 높은 것이 없다(§6.2 ② — 도착끼리는 먼저 온 쪽이 남는다. 같은 날 기원의 둘은 `id` 순으로 먼저 처리된 쪽이 먼저 도착한다). `precedence: null` 후보와 `optional` 은 막지 않는다. `guard` 가 없으면 2027년 수태고지가 성 토요일에, 2035년에는 부활 월요일에 앉는다. 도착한 축일이 그 날짜의 prec 7 후보를 생략시키는 것(절차 ⑤)은 B1 의 `official` 판정에서 다룬다 — 패스는 좌석만 정한다.

**도착했다가 다시 떠나는 날.** `isFree` ② 는 **더 낮은** 도착이 있는 날을 비운 것으로 치므로, 나중 기원의 더 높은 축일이 그 날에 들어올 수 있다. 그러면 루프가 그 날에 이르렀을 때 낮은 도착이 `ranked` 에서 밀려 다시 다음날로 간다 — 그 축일은 그 날짜의 `arrivals` 와 `departures` **양쪽**에 실린다. 실제 사례가 2035·2046 이다: 성 요셉(3.19, 성주간 월, prec 6)이 먼저 부활 8일을 건너 4.2 에 앉고, 뒤에 수태고지(3.25 = 부활대축일, prec 3)가 같은 4.2 로 오면서 요셉을 4.3 으로 민다. `resolveDate("2035-04-02")` 는 요셉을 **한 항목**으로, `status: "transferred_out"` · `from: "2035-03-19"` · `to: "2035-04-03"` · `displacedBy: 수태고지` 로 낸다(§5.5 — 출발이 도착을 대신한다). 4.3 에서는 `transferred_in` · `from: "2035-04-02"`. `from` 은 언제나 **직전** 자리라 계보는 날짜를 거슬러 따라가면 복원된다 — 기원을 한 필드에 따로 싣지 않는다.

**패스가 결정적인 이유 셋.** ① 날짜를 오름차순으로 훑고, `next_day` 는 언제나 **앞으로만** 가므로 어떤 날짜 `d` 에 이르렀을 때 `arrivals.get(d)` 는 완성돼 있다 — 그 뒤에 도착할 것이 없다. ② 연쇄는 이 루프 자체다 — 도착이 그 날짜의 같은 값·낮은 값 고유 후보를 밀어내고(§6.2 ①), 밀린 것이 `transferable` 이면 루프가 `d` 에 이르렀을 때 `ranked` 에서 승자가 아니므로 다시 다음날로 간다. 성탄 3축일이 하루씩 밀리는 것이 정확히 이 경로고 `from` 은 **직전** 날짜를 가리킨다(어린이들의 `from` 은 12.28, `displacedBy` 는 요한). 같은 날을 노리는 도착 둘(2038 마르코·필립보와 야고보가 부활 8일을 건너 5.3 에 함께 닿으려는 경우)은 **기원이 이른 쪽**(마르코 4.25)이 5.3, 다음(필립보·야고보 5.1)이 5.4 다 — §6.2 ② 「도착끼리는 먼저 온 쪽」. ③ 순서는 §6.2 tie-break 로 완전히 정해지고 배열 순서에 기대는 곳이 없다.
