...
- `findReadings(coord)` → `reading_track` · `set_no` 별 **그룹**. `set_no` 정렬은 결정적이어야 하며(기사 배열 순서 금지) **옛 연도판을 뒤로**, 인쇄된 세트 번호 순, 본 독서가 대안보다 앞(ADR-037 §1 개정). 실측 분포(2026-08-29): `set_no` 는 1(841) · 2(81) · 3(1), `set_total > 1` 인 레코드가 163건.
...
| 1 | `principal` | 대축일 7 + 대재일 3(재의 수요일·성 금요일·**성 목요일** — 2026-08-29 확정) |
...
`precedence` 가 `null` 인 후보(collects 4건 — 맥추감사주일·여성선교주일·노동자의 날·거룩한 이름 예수 전야. 성 목요일은 2026-08-29 에 대재일로 확정돼 빠졌다)는 **정렬 최하위**, 승자로 뽑지 않는다. 이 중 노동자의 날·거룩한 이름 예수 전야는 `outranks_sunday` 도 `null` 이다.
...
- **품계와 전례색은 독립 축**이다 — 등급 칸이 비어도 색 칸은 채워질 수 있다. (오래 실제 사례였던 성 목요일은 2026-08-29 에 대재일로 확정돼 더는 예가 아니다. 원칙은 그대로다 — 표의 두 칸은 서로를 함의하지 않는다.)
