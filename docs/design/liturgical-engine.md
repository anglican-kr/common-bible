...
**캐시가 둘인 이유**(2026-09-05 리뷰): 연도 경계 처리(§6.5)가 **임시 패스**를 돌리는데, 그 결과를 `Y−1` 의 정식 항목으로 남기면 안 된다(seed 없이 만든 부분 패스라). **그 임시 패스는 `Y−1` 앵커만 쓰지 않는다**(2026-09-13 정정 — 10차 변경의 파급): `boundary` 0단계가 `optionalObservancesOf(Y)` 까지 깔고, 그중 승천(`easter7_sunday`)은 **부활절 기준**이라 `Y` 의 앵커를 함께 요구한다. 그래도 computus 횟수는 늘지 않는다 — 아래 세 번 안에 이미 `Y` 가 들어 있다. 앵커와 패스를 한 항목에 두면 「앵커는 있고 패스는 없는」 반쪽 항목이 생기고, 뒤의 `resolveDate(Y−1)` 가 그것을 완성된 것으로 재사용할 수 있다. 캐시를 가르면 `anchorCache` 는 어느 경로로든 채워도 되고(순수 파생값), `transferCache` 는 **완전한 패스만** 들어간다 — 부분 상태가 표현될 자리가 없다.

캘린더 월간 뷰가 42칸을 그리며 `resolveDate` 를 42번 불러도 computus 는 **최대 세 번** 돈다 — 한 해 `Y` 의 완전한 패스가 seed 를 위해 `Y−1` 앵커를 요구하고(§6.5 연도 경계), 42칸이 `Y`·`Y+1` 을 걸치면 `Y−1`·`Y`·`Y+1` 이다. 한 해만 걸리면 두 번. `anchorCache` 는 순수 파생값이라 무효화가 필요 없다. `transferCache` 는 선택 봉헌 활성화 집합(§6.2 `activated`)에 의존하는 **유일한** 입력이 있어, 그 설정이 바뀌면 통째로 비운다.
...
function computeTransfers(year, seed /* Y−1 에서 넘어온 arrivals */, activated, boundary = false) {
  ...
  for (const d of daysOf(year)) {          // 1.1 → 12.31, 날짜 순 — 순서가 결과를 정한다
    ...
    for (const c of ranked.slice(1)) {     // 승자를 뺀 나머지, ranked 순 = §6.2 ② 「같은 기원이면 품계 순 → id 순」
      ...
      let dest = nextDay(d);
      while (daysBetween(d, dest) <= MAX_SHIFT && !isFree(dest, c, arrivals, departures)) dest = nextDay(dest);
...
**`isFree(dest, c, arrivals, departures)`** — `c` 가 그 날짜에서 **이길 수 있으면** 빈 날이다. 여기서 「고유 후보」는 본 루프와 **같은 집합**이다 — `properCandidates(dest)` 에서 `departures.get(dest)` 에 있는 활성화된 선택 봉헌을 뺀 것(켜진 승천은 목요일에 없으므로 그 목요일을 노리는 다른 이동을 막지 않는다). 구체적으로 ① 그 고유 후보와 `guard(dest)` 중 `c` 보다 **높은**(유효값이 작은) 것이 없고 — 같은 값의 고유 후보는 도착 우선(§6.2 ①)으로 `c` 가 밀어내므로 막지 않는다 — ② **이미 도착해 있는 후보가 하나도 없다** — 품계를 보지 않는다(§6.2 ② 선착순. 2008 미국 성공회: 요셉이 3.31 을 지키고 수태고지가 4.1). 같은 날 기원의 둘은 품계 순 → `id` 순으로 먼저 처리된 쪽이 먼저 도착한다. `precedence: null` 후보와 `optional` 은 막지 않는다. `guard` 가 없으면 2027년 수태고지가 성 토요일에, 2035년에는 부활 월요일에 앉는다. 도착한 축일이 그 날짜의 prec 7 후보를 생략시키는 것(절차 ⑤)은 B1 의 `official` 판정에서 다룬다 — 패스는 좌석만 정한다.
