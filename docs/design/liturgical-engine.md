...
캘린더 월간 뷰가 42칸을 그리며 `resolveDate` 를 42번 불러도 computus 는 최대 두 번(달 경계에서 두 해가 걸릴 때) 돈다. `anchorCache` 는 순수 파생값이라 무효화가 필요 없다. `transferCache` 는 선택 봉헌 활성화 집합(§6.2 `activated`)에 의존하는 **유일한** 입력이 있어, 그 설정이 바뀌면 통째로 비운다.
...
function computeTransfers(year, seed /* Y−1 에서 넘어온 arrivals */, activated) {
  ...
  for (const d of daysOf(year)) {          // 1.1 → 12.31, 날짜 순 — 순서가 결과를 정한다
    ...
    for (const c of ranked.slice(1)) {     // 승자를 뺀 나머지, ranked 순 = §6.2 ② 「같은 기원이면 품계 순 → id 순」
      ...
      const displacer = ranked.find((w) => outranks(w, c) && (variesByYear(w) || variesByYear(c)));
      if (!displacer) continue;            // 고정일끼리만 겹친 것은 충돌이 아니다 (아래)
      let dest = nextDay(d);
