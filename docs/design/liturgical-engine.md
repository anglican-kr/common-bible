...
**캐시가 둘인 이유**(2026-09-05 리뷰): 연도 경계 처리(§6.5)가 `Y−1` 의 앵커만 쓰는 **임시 패스**를 돌리는데, 그 결과를 `Y−1` 의 정식 항목으로 남기면 안 된다(seed 없이 만든 부분 패스라). 앵커와 패스를 한 항목에 두면 「앵커는 있고 패스는 없는」 반쪽 항목이 생기고, 뒤의 `resolveDate(Y−1)` 가 그것을 완성된 것으로 재사용할 수 있다. 캐시를 가르면 `anchorCache` 는 어느 경로로든 채워도 되고(순수 파생값), `transferCache` 는 **완전한 패스만** 들어간다 — 부분 상태가 표현될 자리가 없다.
...
| `ascensionToPentecost` | 승천 ~ 성령강림 전날 (E+39 ~ E+48) | 1 | §5.3 부활6주 금·토 본기도 병합의 근거(데이터 `_note`) |
...
| **기간 축(§4.10)** | `spansOf` 가 앵커만으로 계산(데이터 불필요) · `transferGuard` = E−7~E+7 정확히 · 색 가드 = `holyWeek` ∪ `easterOctave` · 소재일 제외 = 12.25~세례주일 · `ascensionToPentecost` = E+39~E+48. **세 소비자(§6.3 ① · §6.4 · §6.5)가 같은 함수를 본다**는 것이 케이스의 요지다 |
