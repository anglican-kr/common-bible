...
| **교회력 격자 빈 칸 6개** | 사순5주 나해 · 연중5·8주 나해 · 연중9주 가·다해 · 연중10주 다해 — 원료에 그날 기사가 없다(ADR-037 미결5) | `findReadings` 가 **빈 배열**을 반환한다. throw 하지 않는다. 빈 배열과 "아직 안 불러옴"을 뷰가 구별할 수 있어야 한다 |
| **음력 표는 2025–2050** | `kasi-lunar.json` 26년치(2026-08-03 완료) | 범위 밖 연도는 설·추석 후보를 **생성하지 않는다**. 조용히 건너뛰되 `null` 이 아니라 빈 결과로 |
| **품계 미해소 5건 · 전례색 미해소 2건** | 맥추감사·여성선교·성 목요일 등급 빔, 노동자의 날·거룩한 이름 예수 전야는 표에 행 없음(ADR-036 미결10) | `precedence` 가 `null` 인 후보는 **정렬 최하위**로 두고 승자로 뽑지 않는다. 표에 행이 없는 날은 애초에 후보로 오르지 않는다 |
...
function preloadCalendar() {
  return (calendarPromise ??= Promise.all([...]).then((t) => (calendarTables = t)));
}
```
...
**나머지 규칙 종류**(실측 `_meta.rule_kinds` 6종 + 미사용 2종):
...
2. **한 레코드가 후보 셋을 낳으므로 요일로 갈라야 한다.** 독서·본기도는 사계재를 **지향(志向)별**로 나눠 두었고(「성직자의 성소를 위한 …계재」 등, 계절이 아니라 지향이 축이다), 레코드 이름이 `사순1주 수요일 - 성직자의 성소를 위한 춘계재` 꼴이다. 엔진이 만든 세 날짜를 그 이름·요일과 맞춰야 조회가 성립한다. **이 조인은 아직 검증되지 않았다**(§9 미결).
...
- **`id` 타입이 파일마다 다르다** — readings 는 문자열(`ordinary-9-sun-A-t1-s1`), collects 는 **정수**(1~334). 동점 정렬 키로 쓸 때 숫자·문자 비교를 섞지 않는다.
...
- **`ending` 은 A 324 · C 10 · B 0** — ADR-037 미결1(송영 A/B 구분 규정)이 데이터로도 미해결임이 확인된다.
- **`_review` 가 collects 21건에 붙어 있다** — 본기도 본문이 공식 기도서와 다를 수 있다는 표시(ADR-036 미결8). 엔진은 무시하고 통과시킨다.
...
`season`·`week`·`type` 은 정확히 일치해야 한다(폴백 없음). 총점이 높은 레코드가 이긴다.

```js
function matchScore(record, coord) { … }   // -1 = 불일치
```

**동점 처리를 반드시 결정해 둔다.** 동점이 나오면 데이터 결함일 가능성이 높으므로, 배열 순서에 기대지 않고 **레코드 `id` 순**으로 안정 정렬한다(문자열은 사전순, 정수는 수치순 — §5.2 의 타입 차이). 정렬이 입력 순서에 의존하면 데이터 저장소가 재빌드할 때마다 결과가 흔들린다.
...
**중요한 예외 — `weekday: null` 이 항상 "그 주 평일 공통"을 뜻하지는 않는다.** 앞서 이 문서는 "독서 쪽에서 `null` 이 나올 일은 없다"고 적었으나 **실측이 이를 반증했다**: readings 의 `weekday` 는 403건이 `null` 이다. 다만 그 `null` 들은 폴백이 아니라 **고유명 날짜**다.
...
| 방향 | 키 | 근거 |
|---|---|---|
| **sanctoral ↔ readings/collects** | **`date`(`MM.DD`) + `lunar`** | 고유 본기도 52건 중 날짜 조인은 **52/52 성공**(`date` 50 + `lunar` 2), 이름 조인은 35건만 성공 |
...
- `findReadings(coord)` → `reading_track` · `set_no` 별 **그룹**. `set_no` 정렬은 결정적이어야 하며(기사 배열 순서 금지) **옛 연도판을 뒤로**, 인쇄된 세트 번호 순, 본 독서가 대안보다 앞(ADR-037 §1 개정). 실측 분포: `set_no` 는 1(841) · 2(78) · 3(1), `set_total > 1` 인 레코드가 157건.
...
- **독서 슬롯은 네 개지만 다 있지는 않다.** 실측: `first` 920 · `psalm` 920 · `gospel` 919 · `second` 381. `second` 가 없는 539건은 대개 평일(526)이라 정상이고, `gospel` 이 없는 **단 하나**가 12.31 거룩한 이름 예수 전야다. 뷰가 "마지막 칸이 복음"이라고 가정하면 이 하루에서 깨진다(ADR-037 §1 개정 ②).
- 시편 슬롯의 `bookId` 는 전례시편 `lps`, 송가일 때는 `cant` + `canticle` 키가 붙는다.
- `findCollects(coord)` → 두 축을 **구분해서** 반환한다(ADR-038 §3): 같은 관측일의 자유선택 대체안(`collect_no` — 뷰가 택1 탭)과, 서로 다른 관측일이 겹쳐 따라오는 본기도(뷰가 순서대로 열거).
- **송영은 본문에 없다** — 레코드의 `ending` 이 앱 상수 3종 중 하나를 가리키고, 열거 **마지막 1회만** 부착한다(ADR-036 §3 개정). 엔진은 코드만 넘긴다. 실측은 A 324 · C 10 · **B 0**.
...
| 1 | `principal` | 대축일 7 + 대재일 2(재의 수요일·성 금요일) |
...
`precedence` 가 `null` 인 후보(collects 5건 — 맥추감사주일·여성선교주일·성 목요일·노동자의 날·거룩한 이름 예수 전야)는 **정렬 최하위**, 승자로 뽑지 않는다. 이 중 노동자의 날·거룩한 이름 예수 전야는 `outranks_sunday` 도 `null` 이다.
...
- **품계와 전례색은 독립 축**이다 — 등급 칸이 비어도 색 칸은 채워질 수 있다(성 목요일이 실제 사례).
...
| 결손 내성 | `rule: null` 건너뛰기, 격자 빈 칸 6개에서 빈 배열, `precedence: null` 최하위 |
...
5. **사계재 날짜 ↔ 본문 레코드 조인이 검증되지 않았다** — 규칙은 채워졌지만(§4.3), 엔진이 만든 수·금·토 세 날짜가 지향별로 나뉜 독서·본기도 레코드(16건)와 실제로 맞물리는지는 확인 전이다. 특히 `temporal.md` 가 남긴 관찰 — 본기도는 **계절이 아니라 지향이 축**이라 하계·추계·동계재가 같은 본문을 공유하고, 레코드 대부분이 `weekday: null` 이라 요일 배정이 불명확하다. **A1-b 착수 시 첫 실측 대상.**
...
7. **`national` 4건의 본문 조인이 어긋난다** — `has_proper: true` 인데 날짜로 독서·본기도를 찾으면 2건만 나온다. 이름 조인과 날짜 조인이 갈리는 자리로 보인다(§5.4). 삼일절·광복절 계열이라 급하지 않으나 A1-b 에서 확인할 것.
