...
**총점이 높은 레코드가 이기되, 이기는 것은 「한 건」이 아니라 「한 층」이다.** 최고점 레코드를 **전부** 남기고 그 다음에 그룹으로 묶는다 — 같은 좌표에 **의도적으로 공존하는** 레코드가 있기 때문이다: 자유선택 대체 본기도(`collect_no`, 실측 36 엔트리)와 독서의 `set_no`·`reading_track` 세트(실측 `set_total > 1` 163건). 이들은 `weekday`·`year` 가 동일해 점수도 같으므로, 최고점에서 하나만 골라내면 §5.6 과 ADR-036 §2 의 반환 계약이 깨진다 — 뷰가 택1 탭으로 보여야 할 두 본기도 중 하나가 사라진다.
...
| 방향 | 키 | 근거 |
|---|---|---|
| **sanctoral ↔ readings/collects** | **`date`(`MM.DD`) + `lunar`** | 고유 본기도 52건 중 날짜 조인은 **본기도 52/52 · 독서 51/52 성공**(`date` 50 + `lunar` 2), 이름 조인은 35건만 성공. 독서에서 빠지는 하나는 프란시스 사베리오(12.03) — **고유 본기도 + 공통 독서**라 고유 독서가 없는 게 맞다(articles-supplement 의 `readings: null` 케이스). `lunar` 를 빼고 `date` 만으로 맞추면 설날·추석이 떨어져 나간다 |
...
8. 🔴 **주일 주기(A/B/C)의 기준 연도가 저장소 어디에도 없다** — §4.7 은 「대림 시작 기준 3년 주기」라고만 적었는데, **어느 전례년이 A 인지**가 없으면 3년 주기를 계산할 수 없다. 모듈로 오프셋이 하나만 틀려도 **모든 날짜의 주일 독서가 조용히 어긋난다** — 예외도 오류도 없이 다른 해 본문이 나온다. 정본 전사(`docs/reference/liturgical-calendar-rules.md`)에 주기 규칙 자체가 없고, `coords.py` 는 한글 라벨↔코드 매핑뿐이다. **구현 전에 기준 연도를 기도서·교구 달력에서 확정해 여기 적을 것**(대림 경계 예시도 함께). 데이터에 이미 `year: "A"|"B"|"C"` 가 붙어 있으므로, 확정 후 실제 연도의 주일을 스크랩 좌표와 대조해 검증할 수 있다.
