 ...
 *                           //   transferred_in·transferred_out·commemorated·omitted 에.
 * }} Candidate */
```
...
| `status` | 뜻 | 누가 만드나 |
|---|---|---|
| `proper` | 그 날짜의 고유 후보(아래 ①~④)의 **초기 상태** — PR 2 에서는 전부 이 값이다. B1 은 승자가 아닌 고유 후보를 규칙에 따라 재분류한다: 이동하면 `transferred_out`, `commemorate_only`·C 그룹 「옮기기 어려우면」이면 `commemorated`, prec 7 이면 `omitted`. **재분류 뒤에도 `proper` 로 남는 것**은 ① 승자(`official`) ② 격자 주일·평일 후보(연중 평일 독서 — 어느 규칙도 옮기거나 생략하지 않으므로 「밀린 독서 보존」(ADR-036 §7)이 이 상태로 실현된다) ③ 고정일끼리의 동시 봉헌에서 진 쪽(9.29 설립 기념일 — §6.5, 미결18) ④ `transferable: false` 인 주요축일·대축일이 진 경우(현재 표에서 12.25 같은 대축일은 지지 않으므로 사례 없음 — 생기면 데이터 결함 신호) | PR 2 (초기) · PR 3 (재분류) |
...
function computeTransfers(year, seed /* Y−1 에서 넘어온 arrivals */, activated /* Set<id> — §6.2 활성화 계약 */) {
  ...
  for (const d of daysOf(year)) {          // 1.1 → 12.31, 날짜 순 — 순서가 결과를 정한다
    const proper  = properCandidates(d)    // §5.5 ①~④, 이동 없음
      .filter((c) => !(activated.has(c.observance.id) && optionalDestOf(c, d)));  // 켜진 선택 봉헌은 원래 날짜에 없다
    ...
    for (const o of optionalObservances(d)) {
      if (activated.has(o.observance.id)) {   // 켜진 것은 실제로 옮긴다 — optionals 가 아니라 도착
        push(arrivals,   o.dest, { observance: o.observance, status: "transferred_in",  from: d });
        push(departures, d,      { observance: o.observance, status: "transferred_out", to: o.dest });
      } else push(optionals, o.dest, { observance: o.observance, status: "optional", from: d });
    }
  }
...
**선택 봉헌**(§6.2)은 같은 루프에서 **별도 색인 `optionals`** 에 얹는다 — 원래 날짜의 고유 후보를 옮기지 않고 목적지 주일에 `status: "optional"` 을 **더한다**. `arrivals` 에 섞지 않는 이유: 도착·출발 색인은 「출발 수 = 도착 수 − seed 도착 수」·「도착은 그 날의 승자」 불변식(§7 ⑧)을 지는데, 선택 봉헌은 출발이 없고 승자도 아니라 섞이면 그 불변식이 매년(승천) 깨진다. `isFree` 는 `optionals` 를 보지 않는다(설정이 켜기 전에는 좌석을 차지하지 않는다).
