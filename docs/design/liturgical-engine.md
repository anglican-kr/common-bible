...
2. **한 레코드가 후보 셋을 낳으므로 요일로 갈라야 한다.** 독서·본기도는 사계재를 **지향(志向)별**로 나눠 두었고(「성직자의 성소를 위한 …계재」 등, 계절이 아니라 지향이 축이다) 계절은 본문을 가르지 않는다 — 지향 4종이 곧 본문 4종이다.
   ...
   **2026-08-29(data PR#17)로 조인이 열렸다.** 정본이 `calendar/ember.md` 한 곳으로 모이고 파서가 계절 × 요일로 펴면서 **16건 전부 `weekday` 를 갖는다**(종전 6건). 요일 배정은 네 계절 공통 — **수=성직자 · 금=성직후보자·수도자(2본) · 토=모든 신자**. 레코드 이름은 두 꼴이다: 춘계재만 격자 좌표를 앞에 달고(`사순1주 수요일 - 성직자의 성소를 위한 춘계재`, `season=lent·week=1`), 나머지 셋은 요일을 뒤에 단다(`성직자의 성소를 위한 하계재 수요일`, `season=null`). 엔진은 이름이 아니라 **`(계절, weekday)`** 로 맞추면 된다 — 금요일은 후보가 둘이므로 하나를 고르지 말고 둘 다 내야 한다.
...
5. ~~**사계재 날짜 ↔ 본문 레코드 조인이 검증되지 않았다**~~ **해소 2026-08-29**(data PR#17, ADR-036 미결13) — 요일 배정이 기도서로 확정되고(수=성직자·금=성직후보자·수도자·토=모든 신자, 네 계절 공통) 16건 전부 `weekday` 를 갖게 되어 `(계절, weekday)` 조인이 성립한다(§5.4 참조). 남은 것은 엔진 쪽 구현뿐이다 — **금요일 후보가 둘**이라는 것만 주의.
