...
4. 조회 API 가 좌표가 아니라 **관측일**을 받는다 — `findReadings(observance)`·`findCollects(observance)` → §5.6. §6 은 `findReadings(coord)` 로 적었으나 `LiturgicalCoord` 는 절기 축만 담아 이름일·고정일·음력일로 색인된 본문에 닿지 못한다(2026-08-29 실측). ADR-037 §6 의 계약도 함께 개정했다.
