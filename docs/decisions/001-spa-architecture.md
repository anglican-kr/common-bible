...
> **개정 (2026-05-09): 단순성의 범위 재정의 (ADR-019 채택과 함께)**
>
> 원본 ADR-001의 "백엔드 없는 SPA + 빌드 단계 0"이 핵심이지만, *모든 면에서의 단순성*은 절대 가치가 아니다. 다음 두 축을 구분한다.
>
> | 축 | 결정 |
> | --- | --- |
> | **빌드 단계 0** (브라우저가 원본 `.js`를 그대로 로드) | 유지. 배포는 정적 파일 서빙 + SW 캐시. 번들러 도입은 별도 의제로 보류 |
> | **모듈 namespace 패턴** (multi-script + 글로벌 `window.X`) | 폐기 (ESM 채택). ADR-019 참조 |
>
> 단순성을 절대화하지 않고 *모듈성·의존 그래프 명시·패턴 일관성*과 함께 평가한다. 향후 `lazy load`/`tree-shaking`이 절실해지면 번들러 도입 ADR을 별도로 검토한다 (지금은 아님).
