...
> **개정 (2026-06-09):** 책 목록(탭 스트립) 화면의 그림자 조건부 생략을 **철회**한다. 원래 생략 이유는 "그림자가 탭 스트립에 바로 닿아 시각 잡음"이었는데, 이는 탭 스트립과 스크롤 콘텐츠가 여백 없이 맞붙어 있던 데서 비롯됐다. 고정 영역(`#sticky-group`)에 하단 여백(`padding-bottom: --space-4`)을 둬 탭과 스크롤 콘텐츠 사이에 항상 16px `var(--bg)` 띠가 남도록 바꾸면서(이전엔 `#app`의 스크롤되는 top padding 이 그 역할을 해 스크롤 시 사라졌다), 그림자는 탭이 아니라 그 여백 아래 콘텐츠 경계에서 발현한다. 따라서 책 목록 헤더도 다른 화면과 동일하게 `.scrolled` 시 `--shadow-3` elevation 을 쓴다(특례 제거). 위로 스크롤할 때 책 카드가 탭에 바로 빨려 들어가 답답해 보이던 증상도 함께 해소.
