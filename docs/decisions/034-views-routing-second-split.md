...
| 5b | registry 역전 + closeAllOverlays (미착수) | route()→외부 뷰 dispatch를 `registerView`로 역전(순환 끊기) + route()의 12개 `closeX` → ADR-032 `closeAllOverlays()` | 높은 위험 — search/bookmark/settings 동반 수정. **참고**: 당초 "parsePath 하향으로 audio edge 해소"는 실제론 entangled(parsePath가 division 로직+books 의존) — audio의 `window.parsePath`는 `applyAudioShow`가 route 컨텍스트를 인자로 받게 바꾸는 편이 깔끔, PR5b에서 검토 |
