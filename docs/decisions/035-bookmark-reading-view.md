...
>
> **개정 (2026-06-10):** 폴더 행 읽기 버튼의 글리프·정렬 다듬기.
> - **아이콘:** Material Symbols `auto_stories` → **`play_lesson`**(책 + 재생 삼각형). 폴더 = 하나의 봉독 묶음이므로 "이 과(課)를 차례로 봉독한다"는 능동적 의미가 펼친 책보다 맞다.
> - **정렬:** 북마크 전체 뷰(`renderBookmarksView`) 기준, 폴더 행 트레일링에는 읽기 버튼 + `≡` 재정렬 핸들이 함께 놓인다. 핸들은 이미 헤더 `⋯` 컬럼과 정확히 정렬되므로, 읽기↔핸들 간격을 행 기본값(`--space-3`)에서 헤더 아이콘 간격(`--space-1`)으로 좁혀 읽기 버튼 중심을 헤더 `🛈` 컬럼 아래로 맞췄다(`.bookmarks-view .bm-folder-read-btn` 음수 `margin-right`). 폴더 이름이 `flex:1` 로 핸들을 우측 고정하므로 읽기 버튼만 이동한다. 결과적으로 행의 두 아이콘(읽기·`≡`)이 헤더의 두 아이콘(`🛈`·`⋯`)과 두 컬럼으로 나란히 정렬된다.
>
> **개정 (2026-06-11) — 데스크탑 드로어 폴더 행 버튼 순서:** 데스크탑 북마크 드로어(`#bookmark-drawer`)에서 폴더 행 트레일링 버튼 순서를 **읽기 → 수정 → 삭제** 에서 **수정 → 삭제 → 읽기** 로 바꾼다(사용자 요청). 읽기 버튼은 DOM 상 항상-노출이라 hover-노출인 수정/삭제 묶음보다 앞에 빌드되는데, 항상 켜진 읽기 동작을 hover 쌍 뒤로 보내려고 DOM 을 바꾸는 대신 flex `order`(`#bookmark-drawer-body .bm-folder-read-btn { order: 1 }`, `… .bm-drag-handle { order: 2 }`)로 시각 순서만 조정했다. **드로어 본문에만 스코프** — 전체 `/bookmarks` 뷰는 위 2026-06-10 컬럼 정렬을 그대로 유지한다.
...
> **개정 (2026-07-27) — 절 부분집합 렌더의 식별 키 수정 (ADR-038 A2):** 중복 제거 키가 `number + part + lxx_only` 라 **같은 번호를 가진 절이 첫 하나만 남고 조용히 버려졌다** — 77개 장에서 130절(전례시편 69편의 무번호 `¶` 구절이 앞 절 번호를 물려받고, 그리스어 에스델 추가 본문 31절이 전부 `number:17` 아래에 있다). 번호는 절의 정체가 아니다.
>
> `verse-spec.js` 에 **절 인스턴스 키**(`verseInstanceKey` / `verseInstanceKeys`)를 두어 번호·반절·`alt_ref`·`lxx_only`·`versicle`·`chapter_ref` 를 모두 담고, 중복 제거·DOM id 가 같은 키를 쓴다(원본에 중복 마커가 찍힌 5개 장은 순번 폴백 — 데이터 결함이 절 유실이 아니라 이상한 id 로 degrade). 키는 **장 전체 목록**에서 계산해 어느 발췌에서든 같은 절이 같은 키를 받게 한다.
>
> 함께: `_specCoversVerse`·`_chapterMaxVerse` 를 `verse-spec.js` 로 승격(전례독서 뷰와 공유 — ADR-038 §3), 딥링크 스크롤은 정확한 id 가 없으면 번호가 맞는 첫 span 으로 폴백. `data-vref`(북마크 저장 문법)는 이번 범위 밖 — 하위 절 표기는 ADR-010 verseSpec 문법 확장이 필요하다.
