...
> **개정 (2026-05-11): iOS 17+ 비Safari 브라우저 설치 지원**
>
...
>   - `ios-legacy` — iOS ≤16 + 비Safari. 기존 "Safari로 열어 주세요 + 주소 복사" 메시지 그대로 유지. nudge 비대상.
> - iOS 메이저 버전 판정: `OS X_Y` 토큰 우선, iPad 데스크톱-class UA 마스킹(`MacIntel + maxTouchPoints>1`) 시 `Version/X.Y` 폴백 또는 17 가정.
> - `ios-other` 가이드는 스크린샷 대신 3단계 텍스트 + 감지된 브라우저명("Chrome / Firefox / Edge / Opera / 브라우저") 안내. 브라우저별 공유 버튼 위치가 달라 단일 스크린샷이 부정확하기 때문.
>
> 관련 코드: `js/app/install.js` `getIOSMajor()`, `detectPlatform()`, `buildInstallBody`의 `ios-other`·`ios-legacy` 분기. 테스트: `tests/unit/install.test.js` (iOS 16 / iPad 마스킹 케이스 4건, nudge ios-other 재분류), `tests/e2e/test_install_guide.py` (iOS 17 Chrome / iOS 16 Chrome 분기 케이스).
