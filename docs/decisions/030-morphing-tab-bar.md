...
> - **페이드 도달점 = 캡슐(pill) 75% (06-05 추가 튜닝).** 처음엔 `--scrim-fade-top`을 `--tabbar-reserve + 3rem`(캡슐 윗변보다 한참 위)에 둬 본문이 캡슐 한참 전에 사라졌는데, 그러면 **캡슐 위에 빈 마진이 생겨 탭 바 주변이 휑했다**. → 기준을 캡슐 자체로 옮겨 `--scrim-fade-top = pill 밑변 + pill 높이 × 0.75`(밑변 `--space-1`+safe, 높이 `--touch-target`+`--space-2`×2 = 60px) 로 둔다. 본문이 캡슐 **상단 ¼ 부터** 글래스 너머로 비치고 그 위는 그대로 — 마진 없이 본문이 캡슐까지 채워진다. 오디오 표시 중엔 본문이 처음 만나는 오디오 바에 같은 75% 규칙(`--tabbar-reserve + 0.75 × 3.5rem`).
