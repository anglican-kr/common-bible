...
> **개정 (2026-06-10, 후속⁷): 북마크 ⋯ 메뉴 가로 모드 스크롤.** 후속⁵ 의 글래스 언어를 쓰는 북마크 뷰의 `.title-action-menu`(⋯ 더 보기 — 액션 4~5행 + 정렬 5행)는 `max-height` 가 없어, **가로 모드의 짧은 뷰포트(예: 390px 높이)에서 9행이 화면 아래로 넘쳐 하단 항목(정렬)을 선택할 수 없었다.** → `overflow-y: auto`(가로는 `overflow-x: hidden` 유지해 모서리 클립) + `overscroll-behavior: contain`(가장자리 스크롤이 본문으로 새지 않게) + `max-height` 적용. 메뉴는 `position: absolute` 로 흐름 내 헤더(`#page-title`, sticky 아님)에 앵커돼 실제 top 이 방향·레이아웃에 따라 달라지므로, **열 때 JS 가 트리거 버튼 top → 뷰포트 하단으로 정밀 계산**(`window.innerHeight − 버튼 top − 16px`; 스케일-인 transform 에 흔들리지 않게 메뉴가 아닌 버튼을 측정). CSS `max-height: 80dvh` 는 JS 이전 안전 상한. **부작용 주의:** 메뉴가 flex 컬럼이라 내용이 넘치면 flex 가 항목을 압축해 — 보호 없는 1px 헤어라인(`.title-action-menu-sep`)이 0 으로 무너지고 행도 44px 미만으로 줄 수 있다 → 구분선·행 모두 `flex-shrink: 0` 고정. 스크롤바는 **iOS-26 형식** — iOS Safari 는 네이티브 오버레이 스크롤바(이미 이 형식)를 쓰고 커스텀 `::-webkit-scrollbar` 를 무시하므로, 데스크탑 WebKit/Chromium·Firefox 만 얇은 반투명 pill(트랙 없음, 2px 인셋으로 `--radius-xl` 모서리 비접촉)로 맞춘다(`scrollbar-width: thin` + `scrollbar-color`). 세로 모드는 상한이 넉넉해 스크롤 없이 전부 표시(회귀 없음).
>
> **재계산 (2026-06-10 추가):** 위 정밀 계산을 **열 때 한 번만** 하면, 메뉴를 연 채 기기를 돌릴 때 이전 방향의 cap 이 남는다 — 가로에서 열고 세로로 돌리면 365px 그대로라 **스크롤바가 안 사라지고**, 세로에서 열고 가로로 돌리면 819px 그대로라 **화면을 넘쳐 하단 항목이 안 닿는다**. → 계산을 `sizeMenu()` 로 묶어 **열 때 + 메뉴가 열려 있는 동안 매 `resize` 마다** 재계산(회전 시 `innerHeight` 와 앵커 top 이 동시에 바뀜). 리스너는 `openMenu` 에서 등록, `closeMenu` 에서 해제하고, SPA 내비로 트리거가 떨어지면(`!isConnected`) `sizeMenu` 가 스스로 해제(기존 doc 리스너 self-clean 패턴과 동일).
...
> - 임계값(640/580px, 진행바 6rem)은 진행바 최소 길이 + 컨트롤 폭 산정값으로 실기기 확인 후 조정 가능. 권위 문서 `DESIGN.md` §7 에 반영.
