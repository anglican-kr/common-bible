...
- 일시: 2026-04-25
- 개정: 2026-04-26 (UI 개선), 2026-05-03 (모바일 행 UX), 2026-06-06 (양방향 스와이프 + full-swipe), 2026-06-06 (분절 산문 절 북마크 통합), 2026-06-06 (장 북마크 선택 삭제), 2026-06-07 (삭제 확인 취소 시 행 닫힘 수정), 2026-06-07 (드롭 위치 지시자 가시화 + 행 아이콘 크기 표준화), 2026-06-10 (스와이프 민감도 일관화), 2026-06-10 (정렬 방향 오름/내림 선택)
- 상태: 승인됨
...
> **개정 (2026-06-10): 스와이프 민감도 일관화 — 분류 콘 + 플릭 속도 + 재그랩 커밋 거리.**
> 스와이프가 "어떤 땐 과민, 어떤 땐 둔감"하던 세 원인을 제스처 수학으로 교정. 순수 판정
> 로직을 `SWIPE_GESTURE` 마커 블록(`_classifySwipeAxis`·`_swipeReleaseVelocity`·
> `_resolveSwipeRelease`)으로 분리해 유닛 테스트가 직접 구동한다(ADR-013 패턴).
> 1. **분류(과민·둔감 양쪽의 원인):** 기존엔 첫 5px 이동에서 `|Δx| > |Δy|` 단판 분류 —
>    1px 차이로 모드가 영구 확정돼, 엄지 접촉 노이즈가 스크롤을 스와이프로 가로채거나
>    (과민) 첫 5px 이 살짝 수직이면 이후 수평 100px 도 무시됐다(둔감). → 슬롭 5→**8px**
>    (`SWIPE_SLOP_PX`), 수평 판정에 **각도 편향 ×1.2**(`SWIPE_ANGLE_BIAS`): 수평이 수직의
>    1.2배를 넘어야 스와이프, 수직 우세면 즉시 스크롤 양보(기존 유지), **애매한 대각 콘
>    안이면 미분류로 다음 샘플 대기**(재평가 기회 — 단판 확정 제거).
> 2. **플릭(둔감):** 커밋·스냅이 거리만 봐서 빠른 플릭이 안 먹혔다. → release 시 최근
>    100ms(`SWIPE_VELOCITY_WINDOW_MS`) 표본으로 속도를 구해 **80ms 투영**
>    (`SWIPE_FLICK_PROJECTION_MS`) 후 스냅 임계(±reveal/2) 적용 — 짧고 빠른 플릭으로
>    열기/닫기 가능. 단 **플릭은 커밋(full-swipe 실행)으로 승격되지 않는다** — 삭제 실행은
>    거리 게이트 유지(트위치로 삭제 방지). 드래그 끝에 멈췄다 놓으면 표본 창이 비어 v≈0,
>    순수 위치 스냅(의도 동작).
> 3. **재그랩 커밋(방향 비대칭으로 체감되던 원인):** full-swipe 커밋을 절대 오프셋
>    (`baseOffset + Δx`)으로 판정해, 이미 열린 행(±88px)을 재그랩하면 같은 방향 ~40px 에
>    삭제가 실행되고(과민) 반대 방향은 132px 이 필요했다(둔감). → 커밋은 **제스처 자체의
>    Δx 가 commitPx**(행폭×0.45, 최소 reveal+40)를 넘어야 하고, **절대 오프셋이 reveal 을
>    지났어야**(반대 패널에서 중앙만 갓 넘긴 긴 당김의 오발 방지) 한다. 열림 상태와 무관하게
>    같은 손동작 거리 요구. 스냅(열기/닫기) 임계는 기존 절대 오프셋 기준 유지.
> 좌/우 방향별 임계 차등은 두지 않는다 — 비대칭 체감의 실체가 재그랩 베이스 오프셋이었다.
> 유닛 `tests/unit/bookmark.test.js` `SWIPE_GESTURE` 섹션 13케이스, 제스처 통합은 e2e.

> **개정 (2026-06-10): 정렬 방향(오름/내림) 선택 추가.**
> 그동안 각 정렬 기준은 방향이 고정(제목=가나다순, 날짜 3종=최신순)이었다. ⋯ 더 보기 메뉴의
> 정렬 기준 그룹 **아래에 "오름차순/내림차순" 두 번째 라디오 그룹**을 추가(Apple Music 패턴) —
> 활성 기준에 적용되고 **기준별로 방향을 따로 기억**한다. `직접 정렬(manual)`은 방향 개념이 없어
> 두 행을 **비활성(흐리게)** 처리. 방향은 모드처럼 per-device `localStorage`(`bible-bookmark-sort-dir`,
> `{mode: "asc"|"desc"}` 맵)이며 Drive 미동기(ADR-011 범위 밖, 보기 설정).
> - **현행 동작 보존:** 기준별 **자연 기본값**(제목 `asc`, created/modified/viewed `desc`)을 둬,
>   방향을 한 번도 안 건드린 사용자는 기존과 동일한 순서를 본다.
> - **구현:** `_bookmarkComparator` 를 **오름차순 기준 비교자** `_bookmarkAscComparator` 로 바꾸고,
>   `sortBookmarkNodes` 가 `desc` 일 때 부호를 뒤집는다(폴더-우선 클러스터링은 그대로). 새
>   `getBookmarkSortDir`/`setBookmarkSortDir`(manual·잘못된 값 거부). 메뉴는 `오름/내림` 라디오
>   2행 + 구분선 추가(가로 모드에서 11행이 되지만 ADR-030 후속⁷ 스크롤로 모두 도달).
> - 유닛 `tests/unit/bookmark.test.js` 정렬 방향 7케이스(기본값·per-mode 영속·부호 반전), UI 상태는 e2e.
> - **기준별 설명 문구(혼동 방지):** "오름/내림" 은 추상적이고 그 뜻이 기준마다 뒤집힌다(제목
>   오름=가나다, 추가된 날짜 오름=오래된 순). 그래서 각 방향 행에 **활성 기준에 맞는 설명을
>   라벨 뒤에 muted 로** 단다(제목 `가나다순`/`ㅎ→ㄱ`, 날짜류 `오래된 순`/`최신·최근 순`,
>   최근에 본 `오래전 본 순`/`최근 본 순`) — `syncDirChecks` 가 열 때 갱신, 정적 문구가 아니라
>   기준 따라 바뀐다. 괄호는 **CSS-only**(`::before`/`::after`)라 스크린리더는 단어만 읽고,
>   직접 정렬일 땐 설명이 없어 `:empty` 로 괄호째 숨긴다. HIG: 선행 체크열 + 라벨 형태 유지.

> **개정 (2026-06-11): 재정렬은 ≡ 핸들 전용 — 행 본문 드래그(롱프레스·마우스) 제거.**
> 2026-06-07 에 ≡ 핸들을 들이면서도 기존 진입(터치 롱프레스 500ms, 데스크탑 마우스 즉시
> 드래그)을 **나란히** 남겨, 행 어디를 잡아도 재정렬이 시작됐다. 이 때문에 (1) 스크롤하려고
> 길게 누르면 의도치 않게 드래그가 걸리고, (2) 핸들이라는 명시적 어포던스가 있는데도 본문
> 드래그가 공존해 "여기를 잡으면 이동"이라는 신호가 흐려졌다. → **재정렬 진입을 ≡ 핸들 한
> 곳으로 통일**: `_setupDragHandle` 에서 롱프레스 타이머(`LONG_PRESS_MS`)와 마우스 본문 즉시
> 드래그 분기를 제거하고, 핸들 밖에서 시작한 포인터는 (스와이프 분류를 제외하면) 모두
> 스크롤/클릭으로 양보(`abort`)한다. 핸들 분기(`onHandle`)는 그대로 — 터치·마우스 공통으로
> 이동 즉시 드래그. `moveBookmarkItem`·드롭 지시자·스와이프 로직은 불변. 핸들은 `manual`
> 정렬에서만 노출되므로(2026-06-07) 자동 정렬에선 재정렬 진입 자체가 없다.
> e2e `test_bookmark_swipe.py`: `test_longpress_starts_drag` → `test_longpress_does_not_start_drag`
> (롱프레스가 드래그를 시작하지 않음 확인)로 교체, `test_handle_drag_starts_reorder`(핸들에서
> 즉시 드래그 진입) 신설. dnd e2e 는 `moveBookmarkItem` 직접 호출이라 영향 없음.
