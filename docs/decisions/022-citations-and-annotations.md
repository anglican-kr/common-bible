...
형식: `<short_name_ko> <chapter>:<verse>` 또는 `<chapter>:<verse_start>-<verse_end>`

- `short_name_ko` 는 `books.json` 의 기존 필드를 그대로 사용(예: `창세`, `이사`, `마태`). 별도 약어 사전 불필요.
- 범위 인용: `이사 53:5-7`
- 한 책 내 동일 장 다중 절은 지원 안 함(`이사 53:5,7` 같은 표기는 분리해서 두 개의 `<cite>` 로). 파서·렌더 단순화 우선.
- `tradition` / `lxx` / `mt` 등 변종 속성 없음. 칠십인역 vs 마소라 절 번호 차이는 데이터 측 절 매핑 테이블이 책임지지 인용 태그가 짊어질 영역 아님.

### 3. 절 경계 분리 규칙

`<cite>` 가 마크다운 source 에서 절 경계를 가로지르는 경우(드물지만 가능):

- `split_bible.py` 는 절 단위로 분리하면서 `<cite>` 를 양쪽 절에 복제 보존.
- 두 조각은 같은 `src` 속성을 공유.
- 첫 버전에서는 "같은 인용의 분리된 조각" 임을 마킹하지 않는다(렌더에서 각자 독립적으로 칩 표시 — 인쇄 관습과 일치).
...
- 토글 on:
  - 인용: cite segment 끝에 옅은 회색 칩 `(이사 53:5)`. **이 칩 자체가 링크** — 클릭 시 `/isa/53` 으로 이동(범위 인용은 시작 절). 본문 글자에는 링크 스타일(밑줄·색) 적용 안 함.
...
| `tradition`/`lxx`/`mt` 속성으로 사본 전통 표현 | YAGNI. 칠십인역·불가타 번호 차이는 데이터 측 절 매핑 책임. 인용 태그가 짊어지면 source 가 복잡해짐. |
