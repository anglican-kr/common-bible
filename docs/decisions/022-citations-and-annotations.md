...
- 토글 on (기본):
  ...
  - **인용 바텀 시트 (cite sheet)**: 칩 클릭 시 열리는 모달 시트. 페이지 이동 없이 인용된 본문을 시트 안에서 표시 — 독서 흐름 유지 (절 번호 aria-hidden 결정과 같은 결).
    ...
    - **모바일/데스크탑**: 기존 검색 결과 시트 패턴 재사용. 모바일은 화면 하단에서 끌어올림, 데스크탑은 동일 시트가 적당한 폭으로 표시 (별도 popover 컴포넌트 불필요)
      > **개정 (2026-06-21, 코드 정합):** 위 "데스크탑은 동일 시트가 적당한 폭으로 표시"는 현재 구현과 다르다. cite sheet(`#cite-sheet`)의 데스크탑 표현은 **우측 슬라이드-인 사이드 패널**(`pointer: fine` + `min-width: 769px` 일 때만 — `top: 2.5vh`·`height: 95vh`·좌측 가장자리 폭 리사이즈·`--shadow-drawer`)로, 북마크 드로어(`#bookmark-drawer`)·검색 책 선택 시트(`.book-filter-sheet`)와 **동일한 데스크탑 패턴**(`bm-drawer-in-right`/`out-right`)을 공유한다. 폭이 ≥769px 라도 **터치 태블릿(`pointer: coarse`)은 모바일 바텀 시트를 유지**한다 — 전환 기준은 폭 단독이 아니라 폭 + 포인터 종류다. 오버레이·드래그-리사이즈 공용 배관은 ADR-032, 세 패널 통일 결정은 ADR-033(2026-06-11 개정) 참조. `DESIGN.md` §5·§6 동기화 완료.
