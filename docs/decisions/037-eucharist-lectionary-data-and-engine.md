...
- 마크다운 DSL·파서(`src/liturgical_psalter.py`)·산출 스키마의 권위 출처는 **데이터 저장소 ADR**(D1에서 신설). 요점만: `liturgical/psalter/{n}.md` 편당 1파일, `[N]` 절 / **반행 = 한 줄, 계응 경계 = 개행**(홀수 줄 = 선창, 짝수 줄 = 응답 — 렌더 시 "마지막 줄 아닌 홀수 줄" 끝에 ◯ 표시) / `[-]` = 무번호 후속 구절(원문 `¶`, 렌더 시 ¶ 표시) / 빈 줄 = 연. **입력 문법은 전부 기본 자판 문자** — `◯`·`¶` 같은 전례 글리프는 저장하지 않고 렌더링할 때만 표시한다(자판 입력이 어려움). **표시용 줄바꿈도 저장하지 않는다**(의미 구조만). 파서는 절 블록 줄 수가 홀수면 리포트에 플래그. 템플릿·실전 예 = `liturgical/psalter/TEMPLATE.md`.
