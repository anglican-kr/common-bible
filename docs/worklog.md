...
**OAuth /token 응답에서 X-XSS-Protection 중복 (Google `0` + 우리 `1`)**: Google upstream이 X-XSS-Protection을 명시 비활성화로 보내고 우리는 활성으로 추가하는 모양새가 됨. 이 헤더는 deprecated이고 모던 브라우저는 무시하며 /token 응답은 JS가 body만 소비하므로 실제 영향 0. `proxy_hide_header`로 정리 가능하지만 BFF 코드 복잡도가 늘어 보류.
