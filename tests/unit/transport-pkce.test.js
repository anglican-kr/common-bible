...
test("4. beginRedirectAuthPKCE: location.href에 올바른 OAuth URL 설정", async () => {
  const { transport, location, sessionStorage } = loadTransport({
    location: { pathname: "/gen/3", search: "" },
  });
  await transport.beginRedirectAuthPKCE("client-id-x", "scope-x", { prompt: "consent" });
...
test("5. beginRedirectAuthPKCE: prompt=none이면 silent=true 저장", async () => {
  const { transport, sessionStorage } = loadTransport();
  await transport.beginRedirectAuthPKCE("c", "s", { prompt: "none" });
  const saved = JSON.parse(sessionStorage.getItem("bible-drive-redirect-state-pkce"));
  assert.equal(saved.silent, true);
});

test("6. beginRedirectAuthPKCE: 저장된 email이 있으면 login_hint 추가", async () => {
  const { transport, location } = loadTransport({
    localStorageInit: { "bible-drive-sync-email": "user@example.com" },
  });
  await transport.beginRedirectAuthPKCE("c", "s");
  const url = new URL(location._hrefAssignments[0]);
  assert.equal(url.searchParams.get("login_hint"), "user@example.com");
});

test("7. beginRedirectAuthPKCE: code_challenge가 verifier의 SHA-256 base64url과 일치", async () => {
...
test("8. consumeRedirectCallbackPKCE: callback 없으면 null", () => {
  const { transport } = loadTransport({ location: { search: "" } });
  assert.equal(transport.consumeRedirectCallbackPKCE(), null);
});

test("9. consumeRedirectCallbackPKCE: 정상 callback → {ok:true, code, verifier}", async () => {
  ...
  const result = t2.consumeRedirectCallbackPKCE();
...
test("10. consumeRedirectCallbackPKCE: nonce 불일치 → state_mismatch (state 보존)", () => {
  ...
  const result = transport.consumeRedirectCallbackPKCE();
...
test("11. consumeRedirectCallbackPKCE: 다른 flow의 state면 null (구 implicit과 분리)", () => {
...
test("12. consumeRedirectCallbackPKCE: state 만료 (>10분) → state_expired", () => {
...
test("13. consumeRedirectCallbackPKCE: error param → 해당 reason + returnTo 보존", () => {
  const saved = JSON.stringify({
    nonce: "n", verifier: "v".repeat(43), returnTo: "/gen/5",
    ts: Date.now(), flow: "pkce-v1", silent: true,
  });
  const { transport } = loadTransport({
...
test("14. consumeRedirectCallbackPKCE: state 없음 → no_state", () => {
  const { transport } = loadTransport({
