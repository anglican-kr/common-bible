...
function loadHelpers(initial = {}) {
  ...
  return { ctx, localStorage, loadSearchHistory: reload, pushSearchHistory: push, removeSearchHistory: remove, clearSearchHistory: clear, normalizeSearchQuery: normalize };
}
...
test("loadSearchHistory: caps at SEARCH_HISTORY_MAX (10)", () => {
...
test("pushSearchHistory: dedupes and moves to top (LRU)", () => {
  ...
  h.pushSearchHistory("사랑"); // re-search of older entry
...
test("pushSearchHistory: enforces max of 10 entries", () => {
  const h = loadHelpers();
  for (let i = 1; i <= 12; i++) h.pushSearchHistory(`q${i}`);
  const list = h.loadSearchHistory();
  assert.equal(list.length, 10);
  assert.equal(list[0], "q12");
  assert.equal(list[9], "q3");
  assert.ok(!list.includes("q1"), "oldest entry should be evicted");
  assert.ok(!list.includes("q2"), "second-oldest entry should be evicted");
});
...
test("persists across reloads via localStorage", () => {
  ...
  // simulate a reload by loading fresh helpers with the same backing store
