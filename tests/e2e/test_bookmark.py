...
def test_mobile_header_bookmark_toggle_delete(browser):
    """모바일: 이미 북마크된 장에서 헤더 북마크 아이콘을 누르면 삭제 선택
    모달이 뜨고(저장 모달이 아님), 전체 선택 후 삭제하면 북마크가 제거된다."""
    ...
    try:
        ...
        # Tap header bookmark icon → delete-picker modal, NOT the save modal.
        page.locator(".title-bookmark-btn").click()
        page.wait_for_selector("#bm-chapter-delete-modal:not([hidden])", timeout=2_000)
        assert page.locator("#bm-save-modal").is_hidden(), \
            "save modal must not appear when toggling off an existing bookmark"
        # Nothing pre-selected: the delete button starts disabled.
        assert page.locator("#bm-chapter-delete-confirm").is_disabled(), \
            "delete button must be disabled until a row is selected"

        # "전체 선택" ticks the only row → delete enabled → remove.
        page.locator("#bm-chapter-delete-all").check()
        assert not page.locator("#bm-chapter-delete-confirm").is_disabled()
        page.locator("#bm-chapter-delete-confirm").click()
        page.wait_for_selector("#bm-chapter-delete-modal", state="hidden")
...
def test_mobile_header_bookmark_toggle_delete_cancel_keeps_it(browser):
    ...
    try:
        ...
        page.locator(".title-bookmark-btn").click()
        page.wait_for_selector("#bm-chapter-delete-modal:not([hidden])", timeout=2_000)
        page.locator("#bm-chapter-delete-cancel").click()
        page.wait_for_selector("#bm-chapter-delete-modal", state="hidden")
...
def test_mobile_header_bookmark_toggle_delete_selective(browser):
    """모바일: 한 장에 북마크가 여러 개일 때, 고른 것만 삭제되고 나머지는 남는다."""
    ctx = browser.new_context(viewport=MOBILE_VIEWPORT, user_agent=IPHONE_UA)
    ctx.add_init_script(CLEAR_APP_STORAGE)
    ctx.add_init_script(SUPPRESS_INSTALL_NUDGE)
    page = ctx.new_page()
    try:
        _open_chapter_and_wait(page, "gen/1")
        seed = [
            {"type": "bookmark", "id": "bm-whole", "bookId": "gen", "chapter": 1,
             "label": "창세기 1장", "verseSpec": "all"},
            {"type": "bookmark", "id": "bm-verses", "bookId": "gen", "chapter": 1,
             "label": "창세기 1:1-3", "verseSpec": "1-3"},
        ]
        page.evaluate(f"() => window.syncStoreV2.saveBookmarks({json.dumps(seed)})")
        ...
        # Both bookmarks listed as rows.
        rows = page.locator("#bm-chapter-delete-list .bm-chapter-delete-item")
        assert rows.count() == 2, f"expected 2 rows, got {rows.count()}"

        # Tick only the first row (창세기 1장) → delete.
        page.locator("#bm-chapter-del-0").check()
        page.locator("#bm-chapter-delete-confirm").click()
        page.wait_for_selector("#bm-chapter-delete-modal", state="hidden")

        store = page.evaluate("() => window.syncStoreV2.loadBookmarks()")
        ids = {it.get("id") for it in store}
        assert "bm-whole" not in ids, "selected bookmark should be removed"
        assert "bm-verses" in ids, "unselected bookmark must remain"
    finally:
        ctx.close()
