...
읽기 본문 외에 **하단 내비(모바일 탭 바)** 도 테마색을 시그니처로 쓴다 — 내비의 의도된 강조(ADR-030, 읽기-본문 범위와 별개). **활성 탭/검색은 배경 인디케이터 없이 아이콘 색만 `--theme`**(ADR-030 후속³ 2026-06-05; 이전엔 `--accent` 14% 배경 pill 이었음). 모핑 탭바가 홈 원형으로 접히거나(`.collapsed`) 검색 모핑(`.searching`)할 때 홈 아이콘도 `--theme` 로 틴트한다(동일 언어).
...
> 복구). 활성색 = 아이콘 `--theme`(배경 인디케이터 제거, ADR-030 후속³). 데스크탑 사이드바는 콘텐츠 폭(720)+사이드바(260) 공존에
...
- **아이콘 전용**(라벨 제거, `aria-label`). 탭바·검색·오디오 셋 다 60px 높이 + 동일 floating glass
  (반투명 배경 + `backdrop-filter` + 1px 테두리 + `--shadow-2`). **활성 탭/검색은 배경 인디케이터(pill) 없이 아이콘 색만 `--theme`**(스킴 추종, ADR-030 후속³ — 이전 `--accent` 14% 배경 pill 제거). 홈 원형으로 접히거나(`.collapsed`) 검색 모핑(`.searching`)할 때도 홈 아이콘을 `--theme` 로 틴트 — 동일 시그니처(ADR-030).
