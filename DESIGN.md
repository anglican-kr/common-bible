...
| `--theme` | `#22244a` | `#8ab4d8` | **테마색(스킴 추종)** — 절 번호·단락 기호 + 모핑 탭바 홈 아이콘(ADR-030) |
...
그 외 모핑 탭바가 홈 원형으로 접히거나(`.collapsed`) 검색 모핑(`.searching`)할 때 홈 아이콘을 `--theme`로 틴트한다 — 내비의 의도된 시그니처(ADR-030 개정 2026-06-05). 평상시 활성 탭/검색 틴트는 `--accent`(chrome 중립)다.
...
- **아이콘 전용**(라벨 제거, `aria-label`). 탭바·검색·오디오 셋 다 60px 높이 + 동일 floating glass
  ...
  단, 홈 원형으로 접히거나(`.collapsed`) 검색 모핑(`.searching`)할 땐 안쪽 인디케이터를 빼고 홈 아이콘을 `--theme`(스킴 추종)로 틴트 — 모핑 시 시그니처 강조(ADR-030).
...
- **색상** `--bg · --bg-card · --text · --text-secondary · --accent(고정 차콜) · --accent-light
  · --border · --theme(스킴 추종 — 절 번호·단락 기호 전용)` (+ 스킴/다크 변형)
