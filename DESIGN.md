...
**기본 규칙(fallback): 이 문서·ADR 에 명시된 지침이 없는 사안은 애플 HIG 를 기준으로
결정한다.** 새 컴포넌트·상호작용·레이아웃을 설계할 때 DESIGN.md·ADR 에 규정이 없으면
임의로 정하지 말고 HIG 의 해당 패턴(메뉴·팝오버·시트의 표시/적응, 파괴적 액션 배치,
터치 타깃, 모달리티 등)을 따른다. **HIG 와 다르게 가려면 그 이탈을 ADR 또는 본 문서에
명문화**한다 — 본문 Serif(§3·§11)·중립 chrome 동결(§색상)·헤더 솔리드(ADR-025)처럼
근거가 적힌 의도적 이탈만 허용한다(말없는 일탈 금지).
...
| `--text-tertiary` | `#948e83` | `#707070` | **약화 어포던스 글리프** — 재정렬 핸들 ≡·disclosure·비활성 보조 아이콘. 보조보다 한 단 연하다(있되 물러난 표식, 본문 가독성 대상 아님) |
...
- **iOS 자동 텍스트 확대는 끈다** — `html` 에 `text-size-adjust: 100%`. 기본값(auto)이면
  iOS Safari 가 가로 모드에서 본문 덩어리만 부풀려, rem 절 번호와 em 인라인 마커가 안
  따라와 크기가 어긋나고 본문이 과하게 커진다. 100% 로 고정하면 타이포 스케일은 오로지
  rem 앵커(루트 폰트 + 글자 크기 설정) 하나로만 움직인다. `none` 이 아닌 `100%` 라 사용자
  핀치 줌은 유지된다(WCAG 2.1 AA · 1.4.4).
...
| 절 선택 바 | `#verse-select-bar` | 하단 floating dock(아이콘 pill + 취소 원형 + 부유 카운트 칩) | ADR-010·030 |
| 북마크 선택 바 | `#bm-select-bar` | 절 선택 바와 글래스·치수 공유. 행 선행 선택 원(`.bm-select-circle`) + 하단에 **공유·이동·삭제 pill(내용폭 snug) + 취소 원형**(삭제만 danger). 모바일 전용 | ADR-029 |
| 북마크 이동 모달 | `#bm-move-modal` | 폴더 목록 picker(새 폴더·최상위·폴더들) — 탭 즉시 이동. chapter-delete 모달 카드 패밀리 | ADR-029 |
...
- **가로(`orientation: landscape`) — 탭바 전체 유지 + 오디오 dock 미니** (ADR-030 개정 2026-06-10).
  가로는 폭이 넓어 스크롤 축소(`.collapsed`)에도 탭바를 홈 원형으로 접지 않고 전체를 펴 두고,
  오디오만 dock 행 미니로 둔다(검색 모핑 `.searching` 은 세로처럼 홈 원형 유지). 미니는 '전체
  탭바' 우측~검색 사이 — 탭바 실폭은 `tabbar.js` 가 `:root --tab-bar-w`(보이는 탭 수 × `--dock-control`)
  로 노출해 노트 탭 활성(3→4) 시 자동 정렬. **넓은 폭(≥641px)** 은 미니에 전체 컨트롤(재생·시간·진행·속도)이
  들어가므로 **맨 위에서도 상시 dock**(floating 풀바로 안 올라옴). **좁은 폭**(구형 소형기, 가로 568 등)은
  맨 위 floating + 축소 시 미니이며, 폭 부족 시 **속도(≤640px)→재생 시간(≤580px) 순 차등 숨김** 으로
  진행바 최소 길이(`.audio-progress min-width: 6rem`)를 지킨다. 세로(`portrait`)는 기존대로(홈 원형 +
  재생·진행만).
- **가로 — 이어읽기 배너도 dock pill 로** (ADR-030 개정 2026-06-10). 책 목록 뷰의 이어읽기 배너
  (`.resume-banner`, 평소 헤더 아래 가로 띠)를 가로에서만 하단 dock 행으로 내린다(헤더+배너+탭이
  세로로 쌓여 본문을 가리는 문제 해소). 미니 오디오와 동일 자리(`--tab-bar-w` 정렬) + `--radius-pill`
  + 60px. 배너는 책 목록, 오디오는 chapter 뷰라 같은 자리를 다른 뷰가 공유. **테마색 솔리드 채움 유지**
  (주요 CTA — 글래스 캡슐 사이의 컬러 pill). 세로 무변경.
- **가로 — 검색 모핑도 탭바 유지** (ADR-030 개정 2026-06-10). 검색 버튼을 눌러도 가로에선 탭바를
  홈 원형으로 접지 않고 전체를 유지하고, 검색 입력(`#tab-search-dock`)은 탭바 우측~검색 사이 남은
  공간(미니 오디오·이어읽기 pill 자리)만 채운다. 키보드가 떠도 탭바 유지(세로는 input 풀폭 위해 숨김).
  세로 무변경.
...
- **플로팅 dock 컨트롤은 탭바 기준 한 쌍의 토큰을 공유한다.** 탭 pill 높이·검색/취소 원형
  지름·오디오 바 높이·절/북마크 선택 바 pill·원형이 모두 `--dock-control`(60px =
  `--touch-target` + `--space-2`·2)을, 그 안의 아이콘은 `--dock-icon`(1.7rem, 탭 아이콘
  크기)을 쓴다. 같은 60px 값이 표현식으로 ~14곳에 흩어져 있던 것을 한 토큰으로 수렴.
  pill 안 아이콘은 viewBox 를 글리프에 맞게 크롭해 박스를 꽉 채워 탭 아이콘과 광학 크기를
  맞춘다(스트로크는 탭 아이콘 비율로). **모든 아이콘은 원형 인디케이터(56px) 안에 들어가는
  크기**여야 한다.
- **모든 플로팅 캡슐·원형은 양 끝이 반원(`--radius-pill` + 기본 round corner)이다.** **squircle
  (곡률 큰 superellipse)은 쓰지 않는다** — 코드의 `corner-shape: superellipse(2)` 는 지수 2 =
  정원(원호)이라 squircle 이 아니며 기본 round 와 동일하다(2026-06-07 선언 전량 제거).
- **다중-아이콘 캡슐의 너비·간격은 아이콘 배열에서 파생한다(탭바 모델).** 각 아이콘은
  `--dock-control`(60px) 정사각 슬롯에 중앙 정렬되고(탭 슬롯과 동일 단위 — 별도 마진 토큰
  없이 슬롯 폭이 표준 간격), 캡슐 너비 = 아이콘 수 × 슬롯. 끝 슬롯의 아이콘(⌀ ≈ 슬롯의 절반)은
  반원 캡과 **동심으로 nests** 해 곡면에 얹혀 삐져나오지 않는다(탭바 끝 탭 인디케이터가 캡에
  안기는 것과 동일). 이 너비는 `width: calc(슬롯 × N)` 로 **명시**한다 — `width:auto` 로
  자식 flex-basis 에서 파생하면 WebKit/iOS Safari 가 flex 컨테이너의 intrinsic 너비를 계산할
  때 자식의 flex-basis 를 무시해 캡슐이 아이콘 폭으로 줄고 버튼들이 오른쪽 캡 밖으로 삐져나온다.
...
- **색상** `--bg · --bg-card · --text · --text-secondary · --text-tertiary(약화 어포던스 글리프) · --accent(고정 차콜) · --accent-light
...
- **컨트롤** `--touch-target(44px 고정) · --control-h(36) · --control-h-sm(28) · --icon-btn(40) · --icon-btn-touch(2.45rem, rem 스케일 헤더 아이콘) · --dock-control(60px=touch-target+space-2·2, 플로팅 dock 컨트롤 공통 크기) · --dock-icon(1.7rem, dock 컨트롤 아이콘 — 탭바 기준) · --bm-row-icon(1.6rem, 북마크 트리 행 선행 아이콘 — rem 스케일) · --list-row-h(3.5rem, 목록 행 높이 표준 — 북마크·후속 목록 공유)`
