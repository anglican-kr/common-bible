...
- **경의(Deference)** — 장식이 콘텐츠를 가리지 않는다. 그림자·테두리는 깊이를 암시하는
  최소한으로. frosted glass(반투명 재질)는 iOS toolbar tinting 비동기 문제로 **의도적
  미적용** (ADR-025).
...
| `--accent` | `#22244a` | `#8ab4d8` | 주 강조(버튼·링크·활성 상태) |
| `--accent-light` | `#353766` | `#a8cce8` | 강조 변형 |
| `--border` | `#e0dcd6` | `#3a3a55` | 구분선·테두리 |
| `--verse-num` | `#5a5a78` | `#8ab4d8` | 절 번호 |
| `--paragraph-mark` | `#4a4a65` | `#6a94b8` | 단락 표지(¶) |

### 색상 스킴 (accent 4종)

`[data-color-scheme="red|green|purple"]` 로 accent 계열을 교체한다 (기본 네이비 포함 4종).
각 스킴은 라이트/다크 변형을 모두 가진다. **ON/활성 상태는 항상 `--accent` 를 참조**해
선택한 스킴을 자동으로 추종한다 — 컴포넌트에 색을 하드코딩하지 않는다.
...
- **포커스 링·은은한 강조 배경**은 `color-mix(in srgb, var(--accent) N%, transparent)`
  ...
  한다.
- accent 위 텍스트는 `#fff` (대비 확보). 다크 + 밝은 accent 조합에서 hover 시 텍스트는
  어두운 색으로 뒤집는다 (기존 설정 버튼 패턴).
...
- **frosted glass 미적용**: 반투명 재질은 iOS 26 toolbar tinting / PWA status bar 비동기
  문제로 쓰지 않는다. 불투명 표면 + elevation 으로 깊이를 표현.
...
- **HIG 44pt 최소 탭 영역.** 아이콘 버튼은 시각 크기가 작아도 hit-area 를 ≥44px 로 확보
  (`--touch-target`). rem 기반 헤더 아이콘은 기본 폰트에서 44px 에 도달하도록 박스를 잡고,
  큰 폰트에서는 rem 으로 함께 커진다. 고정 크기 컨트롤은 `min-width/min-height:
  var(--touch-target)` 로 바닥을 깐다.
...
| 토큰 | 값 | 용도 |
|---|---|---|
| `--duration-fast` | `0.18s` | 헤더 그림자 페이드·미세 전환 |
| `--duration-base` | `0.25s` | 시트·팝오버 등장 |
| `--duration-slow` | `0.4s` | 큰 전환 |
| `--ease-out` | `cubic-bezier(0.25, 0.1, 0.25, 1)` | 기본 감속 곡선 |
...
| **frosted glass(반투명 재질)** | ⛔ 이탈 | iOS toolbar tinting / PWA status bar 비동기 문제 (ADR-025) |
...
- **색상** `--bg · --bg-card · --text · --text-secondary · --accent · --accent-light
  · --border · --verse-num · --paragraph-mark` (+ 스킴/다크 변형)
...
- **모션** `--duration-fast/base/slow · --ease-out`
