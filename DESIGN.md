...
읽기 본문 외에 **하단 내비(모바일 탭 바)** 도 테마색을 시그니처로 쓴다 — 내비의 의도된 강조(ADR-030, 읽기-본문 범위와 별개). **활성 탭/검색 = 아이콘 색 `--theme` + 56px 정원 인디케이터(테마색 14% 틴트, 다크 검색 18%)**(ADR-030 후속⁴ 2026-06-05). 인디케이터 지름 56px = 탭 바 높이 60px − 상하 2px 인셋. 모핑 탭바가 홈 원형으로 접히거나(`.collapsed`) 검색 모핑(`.searching`)할 때는 60px 캡슐 자체가 인디케이터라 안쪽 56px 틴트는 빼고 홈 아이콘만 `--theme` 로 틴트한다(이중 원 회피).
...
- **아이콘 전용**(라벨 제거, `aria-label`). 탭바·검색·오디오 셋 다 60px 높이 + 동일 floating glass
  (반투명 배경 + `backdrop-filter` + 1px 테두리 + `--shadow-2`). **활성 탭/검색 = 아이콘 색 `--theme` + 56px 정원 인디케이터**(테마색 14% 틴트, 스킴 추종 — ADR-030 후속⁴). 인디케이터 지름 56px = 바 높이 60px − 상하 2px 인셋. 홈 원형으로 접히거나(`.collapsed`) 검색 모핑(`.searching`)할 땐 60px 캡슐 자체가 인디케이터라 안쪽 틴트를 빼고 홈 아이콘만 `--theme` 로 틴트(이중 원 회피).
...
- **색상** `--bg · --bg-card · --text · --text-secondary · --accent(고정 차콜) · --accent-light
  · --border · --theme(스킴 추종 — 절 번호·단락 기호 + 내비 활성 탭/검색·모핑 홈)` (+ 스킴/다크 변형)
