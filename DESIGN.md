...
| `--theme` | `#22244a` | `#8ab4d8` | **테마색(스킴 추종)** — 절 번호·단락 기호·FAB 전용 |

### 색상 스킴 (테마색 4종)

`[data-color-scheme="red|green|purple"]` 는 **`--theme` 하나만** 교체한다 (기본 네이비 포함
4종, 각 라이트/다크 변형). 테마색이 칠해지는 곳은 **절 번호·단락 기호·FAB
세 곳으로 한정**한다 (ADR-028 개정 2026-06-03, 오디오 플레이어는 2026-06-05 중립화로 제외). 진한 채도로 통일한다.
...
- **색상** `--bg · --bg-card · --text · --text-secondary · --accent(고정 차콜) · --accent-light
  · --border · --theme(스킴 추종 — 절 번호·단락 기호·FAB 전용)` (+ 스킴/다크 변형)
