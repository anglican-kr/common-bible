...
| `--danger` | `#c0392b` | `#c0392b` | **파괴적 액션**(삭제·캐시 비우기) — 채움 버튼·체크박스 accent·강조 텍스트. 라이트/다크·스킴 불변 |
...
- **파괴적 액션은 `--danger` / `--danger-strong`** — 삭제·캐시 비우기처럼 되돌릴 수 없는
  동작의 시그널이다. 채움 버튼(`background: var(--danger)`, hover `--danger-strong`)·삭제
  체크박스(`accent-color: var(--danger)`)·강조 텍스트가 공유한다. `--accent`(중립 chrome)·
  `--theme`(스킴)와 **역할이 다른 별도 축**이라 스킴·라이트/다크에 흔들리지 않는 고정값이고,
  한 화면에 파괴 색이 여럿이면(예: 삭제 picker 의 행 체크박스 + "전체 선택" 토글) **모두
  같은 토큰**을 써 일치시킨다. `--danger` 채움 위 텍스트는 빨강 대비를 위해 예외적으로 `#fff`
  를 유지한다(`var(--bg)` 규칙의 유일한 예외). 새 파괴 UI 는 `#c0392b` 리터럴을 박지 말고
  토큰을 참조한다 (ADR-028 개정 2026-06-06).
...
- **색상** `--bg · --bg-card · --text · --text-secondary · --accent(고정 차콜) · --accent-light
  · --border · --theme(스킴 추종 — 절 번호·단락 기호 + 내비 활성 탭/검색·모핑 홈) · --danger/--danger-strong(파괴적 액션, 고정)` (+ 스킴/다크 변형)
