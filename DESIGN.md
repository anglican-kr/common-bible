...
- **컨트롤** `--touch-target(44px 고정) · --control-h(36) · --control-h-sm(28) · --icon-btn(40) · --icon-btn-touch(2.45rem, rem 스케일 헤더 아이콘) · --dock-control(60px=touch-target+space-2·2, 플로팅 dock 컨트롤 공통 크기) · --dock-icon(1.7rem, dock 컨트롤 아이콘 — 탭바 기준) · --bm-row-icon(1.6rem, 북마크 트리 행 선행 아이콘 — rem 스케일)`
