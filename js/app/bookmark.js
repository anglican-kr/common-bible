...
// ── Bookmark select-delete mode state (ADR-029 개정 / ADR-010) ──
// In-place multi-select on the mobile /bookmarks full view; the enter/exit +
// cascade logic lives in the BOOKMARK_SELECT block below. State stays here (read
// by the gesture handler + tree builders): `_bmSelected` holds only EXPLICIT
// ticks; a row under an explicitly-ticked folder is "covered" (derived, not
// stored) so the folder owns its subtree.
let _bmSelectMode = false;
/** @type {Set<string>} */
const _bmSelected = new Set();

// Inject the gesture engine's two runtime hooks (ADR-034 후속): re-render the
// mounted bookmark surface after a reorder, and read the live select-mode flag
// so swipe/drag suppress while multi-select is active. _rerenderActiveBookmarkTree
// is a hoisted function declaration below; the flag is read via closure.
initBookmarkGestures({
