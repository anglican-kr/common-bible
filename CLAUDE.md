...
- 완료: TypeScript 점진 도입 — `// @ts-check` + JSDoc + tsconfig --noEmit (ADR-012, 2026-05-06)
  - 1차 적용: `js/sync/*` + `js/drive-sync.js` + `js/search-worker.js`
  - **2차 적용 (2026-05-09~10)**: `js/app.js`(부트스트랩 ~283줄) + `js/app/*.js` 9개 모듈 모두에 `// @ts-check` 영구 활성화. ADR-018 모듈 분할 1~8과 동행. 임시 `tsconfig.app.json` 삭제, 메인 `tsconfig.json` 단일 검증
  - 검증: `npx tsc -p tsconfig.json --noEmit`, `npx tsc -p tsconfig.worker.json --noEmit` 모두 0 error
- 완료: app.js 모듈 분할 — Phase 1~8 (ADR-018, 2026-05-09~10)
  - `js/app.js`: 6,082 → 283줄 (**95% 감소**). 9개 도메인 모듈로 분할: `app/helpers.js` / `storage.js` / `settings-ui.js` / `install.js` / `search.js` / `reading-context.js` / `bookmark.js` / `views-routing.js`
  - 모듈 시스템: ESM 일괄 채택 (ADR-019, 2026-05-09). 빌드 단계는 추가 안 함
  - 잔류 `app.js`: app-main 부트스트랩 + 접근성 keydown + Audio cache LRU 소프트캡 + Service Worker 등록
  - 살아있는 설계 문서: `docs/design/app-modularization.md` (진행 일지 포함)
- 완료: 클라이언트 JS 유닛 테스트 (ADR-013, 2026-05-06~11)
  - sync: `state-machine.test.js`(42) + `refresh-store.test.js`(13) + `transport.test.js`(23) + `audio-cache.test.js`(14)
  - app: `storage.test.js`(83) + `helpers.test.js`(31) + `install.test.js`(40) + `search.test.js`(69) + `bookmark.test.js`(104) + `views-routing.test.js`(54)
  - Node 24 `--test` + 자체 vm 하네스, **473 케이스 통과** (2026-05-11 시점)
  - **app 레이어 no-jsdom 영역 1차 완료** (2026-05-11): PR #106 storage + #108 helpers + #109 views-routing popover/initCompactHeader + #110 install INSTALL_STATE/NUDGE + #112 bookmark HREF/ACTIVE/IMPORT_EXPORT + #113 search HISTORY_CONTROLLER. 245 → 473 (93% 증가)
  - ADR-013 개정 (PR #105, 2026-05-11): jsdom dual-track 허용 — layout/Canvas/touch gesture 의존 영역에 한해 happy-dom/jsdom 도입 가능. 0 의존성 → "Node 외 0 의존성". 남은 미커버(settings-ui Canvas/Font·Pull-to-refresh·startScrollTracking·각 모달 UI 등)는 jsdom 도입 후 별도 의제
  - CI: `.github/workflows/test.yml` `unit` job — `node --test tests/unit/*.test.js` (PR #111에서 push·pull_request 트리거 중복 정리)
