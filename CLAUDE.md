...
- 완료: Google Drive 자동 동기화 — Phase 2a~2f (ADR-011)
  ...
  - Phase 2h (2026-05-06): 데스크탑 FedCM/One Tap 카드 회피 — 저장된 email hint가 있으면 IDENTIFYING(`google.accounts.id.prompt()`)을 건너뛰고 AUTHENTICATING으로 직행. `silentAuthInFlight` ctx 플래그로 silent TOKEN_FAIL은 스낵바 없이 NEEDS_CONSENT로 정착
