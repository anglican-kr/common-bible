...
js/
  ...
  drive-sync.js         ← Google Drive 동기화 모듈 (GIS OAuth, ADR-011)
...
- 완료: Google Drive 자동 동기화 — Phase 2b (ADR-011, `feat/drive-sync`)
  - `js/drive-sync.js`: GIS Token Flow, `_accessToken` 메모리 전용, debounce 업로드, merge-by-updatedAt
  - 보안 감사 완료 (`docs/audit/2026-05-02-171111.md`): Critical 0, Medium 2건 수정
  - 미결: Google OAuth 앱 검수 제출, Drive API 네트워크 오류 재시도 전략
- 완료: iOS FedCM/One Tap 인증 — Phase 2d (ADR-011, 2026-05-05)
  - `js/sync/transport.js`: `initIdentityClient`/`promptIdentity`/`parseIdToken` 추가
  - `js/sync/state-machine.js`: `IDENTIFYING` + `NEEDS_CONSENT` 상태 분리, `requestAccessToken`은 사용자 제스처 컨텍스트에서만 호출
  - 효과: iOS Safari "팝업 윈도우를 열려고 시도" 차단 안내 미발생
- 완료: FedCM-mandatory deprecation 마이그레이션 — Phase 2e (ADR-011, 2026-05-05)
  - `prompt()` 콜백 제거로 GSI deprecation 경고 + AbortError 콘솔 노이즈 해소
  - timeout 폴백 폐기 — 다계정 사용자가 FedCM 다이얼로그에서 결정하는 시간 보장 (Phase 2d 회귀 fix)
  - 사용자 escape hatch는 설정의 항시 노출되는 "연결" 버튼
- 완료: 검색 UI 재설계 — 컴팩트 모달 → 결과 시트 + `in:` 연산자 (ADR-005, 2026-05-05)
  - `#search-sheet[data-state="compact"|"expanded"]` 상태 모델로 진입 2단계 분리
  - Enter 시 `blur → rAF → expanded`로 키보드 닫힘과 시트 확장이 220ms CSS 트랜지션으로 동시 진행
  - `in:<book-alias>` 연산자: `parseQuery` + `restrictBooks` Set, unmatched 별칭은 검색 차단 + inline notice
  - `+ in:` 칩(`#search-sheet-chips`): `pointerdown.preventDefault`로 IME 깜박임 방지, 커서를 `:` 뒤에 위치
