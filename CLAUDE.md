...
- **테스트 체계 완료** — ADR-004 데이터 파이프라인(Level 1-3, data 저장소) + e2e + ADR-013 유닛 505 케이스. 유닛은 vm + 수동 스텁(0 의존성), DOM-heavy 영역은 e2e가 책임
...
- **PWA 버전·캐시 무효화 재설계** — ADR-021, 2026-05-13. SHELL_CACHE 는 `version.json`에서 파생 (`sw-version.js` importScripts), DATA/AUDIO_CACHE 는 콘텐츠 해시 매니페스트(`bible-manifest.json`·`audio-manifest.json`) diff 로 항목 단위 lazy 무효화. 데이터 저장소 CI 가 main 머지 시 파이프라인 + 매니페스트 자동 빌드 + `[skip ci]` 커밋백. release.py 가 git 자동 commit (M8 해소). Phase 3 webhook 자동화 후속
