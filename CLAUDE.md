...
테스트마다 `loadMachine()`이 새 vm 컨텍스트에 `js/sync/state-machine.js`를 로드하므로 클로저 상태가 케이스 간에 새지 않는다. 2026-05-11 시점 **309 케이스 통과** — sync 92 (state-machine 42 + refresh-store 13 + transport 23 + audio-cache 14) + app 217 (storage 83 + search 36 + bookmark 70 + views-routing 28). storage 영역은 PR #106(2026-05-11)에서 검색 이력 외 전 영역(읽던 위치·오디오·외관 설정·북마크·설치 안내·영구 저장 요청)까지 확장.
...
- 완료: 클라이언트 JS 유닛 테스트 (ADR-013, 2026-05-06~11)
  ...
  - app: `storage.test.js`(83) + `search.test.js`(36) + `bookmark.test.js`(70) + `views-routing.test.js`(28)
  - Node 24 `--test` + 자체 vm 하네스, **309 케이스 통과** (2026-05-11 시점)
  - ADR-013 개정 (PR #105, 2026-05-11): jsdom dual-track 허용 — layout/Canvas/touch gesture 의존 영역에 한해 happy-dom/jsdom 도입 가능. 0 의존성 → "Node 외 0 의존성"
