...
Node 자체 테스트 러너 + 자체 vm 하네스. 의존성 0, CI 자동 실행. **878 케이스 통과** (2026-09-17). 파일 명명 컨벤션: `tests/unit/<source-basename>.test.js`, 한 모듈 = 한 테스트 파일 — 예외는 **실데이터 의존 케이스**다(공개 CI 의 `Unit tests` 잡이 비공개 `data/` 서브모듈을 받지 않는다). 그 표를 읽어야 하는 케이스는 `tests/unit/<source-basename>.data.test.js` 로 갈라 `haveData` skip 가드를 달고 `engine-data.yml`·`sync-data.yml` 에서 돌린다(선례 `liturgical-engine.data.test.js`, 설계서 §7). 내부 영역은 `// ── <영역> ──` 섹션. 상세는 ADR-013.
...
- 문서: 한국어 기본
- 코드 주석: **영어·한국어 모두 허용** — 언어는 규칙이 아니라 내용이 정한다.
  - 코드가 왜 그렇게 도는지(경계·자료구조·성능)는 **영어**가 자연스럽다 — 주변 식별자와
    같은 어휘를 쓴다.
  - 도메인의 **근거와 결정 이력**은 **한국어**로 적는다 — 전례 용어가 한국어고, 기도서·
    사용자 확인 날짜를 남기는 자리라 문서 언어를 따라야 읽힌다.
  - 규칙에 맞추려고 번역하지 말 것 — 번역하면 근거가 흐려진다.
- 접근성: WCAG 2.1 AA 준수
...
# 작업 + 커밋
# 원장 작성: docs/changes/<YYYY-MM-DD>-<slug>.md (형식은 docs/changes/README.md) + 커밋
git push -u origin <topic-branch>
gh pr create --title "..." --body-file docs/changes/<YYYY-MM-DD>-<slug>.md
# 훅이 원장에 pr: N 을 적어 준다 → 커밋·push
# CI(Unit tests + Change ledger) 통과 확인 후
...
"지금 무엇이 동작하는가"의 권위 출처는 [`docs/status.md`](docs/status.md) — 기능 단위 구현 현황. 구현 PR 머지 시 그 문서를 갱신한다(위 ADR 워크플로우 §3). 상세 결정은 각 ADR + [`docs/architecture.md`](docs/architecture.md) 부록 A 인덱스, 변경 단위 기록은 `docs/changes/`(변경 원장), 미해결 이슈·후속 백로그는 `docs/known-issues.md`.
