...
- **모바일 하단 탭 바 완료** — ADR-029, 2026-06-03. 흩어진 내비(헤더 홈/설정 버튼·검색 FAB·북마크 드로어·설정 팝오버)를 모바일(≤768px) 하단 탭 바로 통합. **홈·검색·북마크·설정 4탭**(`노트`는 후속 슬롯 예약), 각 탭=전체화면 라우트 뷰(`/`·`/search`·`/bookmarks`·`/settings`; 빌더 재사용). **iOS 2026 Liquid Glass** 플로팅 캡슐(frosted glass `::before` 레이어로 Safari home-indicator 틴팅 회피, `corner-shape:squircle`+`border-radius:26px` 폴백, 상시 표시·스크롤 축소 미채택 — iOS 27 회귀 반영). 활성 탭=아이콘 색상 `--theme` + 56px 정원 인디케이터(테마색 14% 틴트 — ADR-030 후속⁴ 2026-06-05; 검색 버튼 활성도 동일. 후속³ 에서 인디케이터를 뺐다가 후속⁴ 에서 56px 테마 틴트로 복원). SPA 전환은 기존 전역 `<a>` 인터셉터, 활성 동기화는 `route()`의 `syncTabBarActive()`. 검색 FAB 전면 제거, 모바일 헤더 홈·설정 버튼 숨김(읽기 뒤로·챕터 북마크 추가는 보존). 오디오 미니플레이어는 탭 바 위 플로팅 둥근 바로 스택, 절 선택 모드 시 탭 바 숨김, `--tabbar-reserve`로 콘텐츠 하단 패딩. 데스크탑(≥769px) 탭 바 미표시 — 기존 헤더/오버레이 유지(사이드바는 ≥~1024px 후속). 북마크 전체뷰는 읽기-문맥 액션 제외, 전역 관리만 — **"⋯"(더 보기) 단일 버튼의 iOS 26 풍 팝업 메뉴**에 새 폴더·내보내기·가져오기 3종 통합(별도 "+" 제거, Apple Music 패턴; 큰 곡률 squircle·full-bleed 행·그룹 사이 hairline 1줄·후행 글리프·44px 터치타깃·리퀴드 글라스, ADR-029 개정 2026-06-05) + **목록 행 여백 확대**. **⋯ 메뉴 정렬 그룹**(직접 정렬·제목·추가된 날짜·최근에 본 날짜·수정한 날짜 `menuitemradio`)으로 정렬 — 표시 전용 `sortBookmarkNodes`(폴더 먼저, manual 은 드래그 순서 보존), 정렬 설정·최근 본 시각은 기기별 `localStorage`(동기화 X), 수정시각은 객체 `updatedAt`(동기화). 자동 정렬 시 드래그 핸들 비활성(ADR-029 개정 2026-06-06). **읽기 헤더 북마크 버튼은 미저장 장에서 책갈피+"+" 배지(추가 어포던스), 저장된 장은 채워진 아이콘 유지; 빈 목록은 아이콘+제목+추가 방법 안내**(ADR-029 개정 2026-06-06). 모바일에서 **저장된 장의 헤더 북마크 토글-오프는 "이 장 전부 삭제" 확인이 아니라 장 안 북마크를 골라 지우는 선택 모달**(`#bm-chapter-delete-modal`) — 체크박스 목록 + tri-state "전체 선택", 기본 미선택·`삭제 (N)` 카운트, 순수 함수 `_selectAllState`·`_deleteBtnLabel`(ADR-010 개정 2026-06-06 후속). **모바일 행 스와이프는 양방향 edge-flush 액션**(iOS 관례 — ← 삭제(trailing) / → 수정(leading), 콘텐츠 슬라이드 모델, full-swipe 즉시 실행; 겹친 액션 탭 hit-test 버그는 `pointer-events`로 수정; ADR-010 개정 2026-06-07). **직접 정렬(manual) 모드에선 각 행 trailing 에 ≡ 재정렬 핸들**(`_buildDragHandle`, `bm-sortable` 토글로 노출, 핸들에서 즉시 드래그; 자동 정렬에선 숨김 — ADR-010·029 개정 2026-06-07). **⋯ 메뉴 맨 아래 파괴색 "삭제"** 항목은 북마크·폴더를 한 번에 골라 지우는 다중 선택 모달(`#bm-bulk-delete-modal`)을 연다 — 트리 전체를 pre-order 체크박스 목록으로 펼치고(tri-state 전체 선택·폴더 cascade), "삭제 (N)"가 공유 확인 알림(`bm-confirm`)을 거쳐 삭제; 순수 헬퍼 `_flattenBookmarkTree`·`_descendantIds`(ADR-029 개정 2026-06-06). 추가는 읽기 화면 전용이라 전체뷰엔 추가 버튼이 없어, **⋯ 왼쪽 정보 버튼 🛈**(중립 차콜, 탭 시 빈 상태와 같은 문구의 안내 팝오버 `.title-action-popover`)로 기존 북마크가 있어도 추가 방법을 안내 — HIG "트리거를 덮지 말 것"에 맞춰 ⋯ 메뉴와 달리 버튼 아래로 떨어짐(ADR-029 개정 2026-06-06, `test_bookmark_add_help.py`). 라우팅/내비는 e2e 책임(ADR-013)
...
- **성서 목록 탭 통합 + 헤더 내비 재설계 완료** — ADR-024, 2026-05-29. 첫 페이지·구약·외경·신약을 탭 한 페이지(`renderBookList(books, activeDivision)`)로 통합, 탭은 외경 설정 반영. 읽기 헤더는 홈 버튼(`buildHomeBtn`, 책의 구분 탭으로) + 브레드크럼 제거 + 설정 버튼 반응형 배치(데스크탑 상단줄 / 모바일 제목 헤더). 구분 탭은 `#sticky-group` 안에 sticky 고정 + 슬라이드 인디케이터(트랙 동색 칩 + elevation 그림자, 윤곽선 중립화 — ADR-024 개정 2026-05-30), 구약 소분류는 제거해 세 탭 평면 통일 (ADR-024 개정 2026-05-29). **홈 버튼 클릭 시 책 목록의 "읽던 책" 항목에 포커스** — `buildHomeBtn(target, ariaLabel, focusBookId)` 가 1회성 `setPendingBookFocus`, `renderBookList` 가 `focusPendingBook` 으로 소비(`data-book-id` 앵커 `focus`+중앙 `scrollIntoView`), 맥락 유지 접근성 (ADR-024 개정 2026-06-07)
