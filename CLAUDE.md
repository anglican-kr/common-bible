...
- `data/` — **서브모듈 `common-bible-data`** 마운트 위치 (73권 JSON `bible/`, 검색 인덱스 4종 `search-{meta,ot,nt,dc}.json`, 오디오 nested 서브모듈 `audio/`, 마크다운 원본 `source/`, Python 파이프라인 `src/`, 데이터 검증 테스트 `tests/`)
- `scripts/` — `release.py`(version/캐시 bump), `serve.py`(SPA-aware 로컬 서버), `generate_splash.py`(iOS 스플래시, ADR-007)
...
마크다운 수정 흐름:
1. `common-bible-data` 저장소에서 `source/*.md` 편집 → 파이프라인 재실행 → 산출물 commit + push
2. 본 저장소에서 `git submodule update --remote data` → `data` 포인터 bump commit
3. `common-bible-server` 저장소의 `deploy.sh dev` → dev 검증 → `deploy.sh promote`
...
- **테스트 체계 완료** — ADR-004 데이터 파이프라인(Level 1-3, data 저장소) + e2e + ADR-013 유닛 479 케이스. 유닛은 vm + 수동 스텁(0 의존성), DOM-heavy 영역은 e2e가 책임
