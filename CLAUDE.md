...
js/
  ...
  drive-sync.js         ← Google Drive 동기화 모듈 (GIS + iOS 풀페이지 리디렉션, ADR-011)
  ...
  sync/
    state-machine.js    ← 동기화 상태 머신 (Phase 2c~2f, ADR-011)
    ...
    refresh-store.js    ← OAuth refresh token 암호화 IDB 저장소 (Phase 2h)
css/
  style.css             ← 메인 스타일
assets/
  icons/
...
테스트마다 `loadMachine()`이 새 vm 컨텍스트에 `js/sync/state-machine.js`를 로드하므로 클로저 상태가 케이스 간에 새지 않는다. Phase 2h 단계 4 시점 62 케이스(state-machine 26 + refresh-store 13 + transport-pkce 23) 통과 — GIS 분기 시나리오가 사라지면서 state-machine 케이스 수는 줄었지만 PKCE 단일 경로의 race·콜백·refresh 회복이 모두 회귀 방어됨.
...
- 완료: Google Drive 자동 동기화 — Phase 2a~2f (ADR-011)
  ...
  - 진행 중 — **Phase 2h (2026-05-06~)**: Implicit Flow → Authorization Code + PKCE + refresh token 마이그레이션. ADR-011 원래 결정으로 회귀해 desktop·iOS 단일 경로 통일
    - 단계 1 ✅ 머지: `js/sync/refresh-store.js` AES-GCM 암호화 IndexedDB (PR #52)
    - 단계 2 ✅ 머지: `transport.js` PKCE 유틸 + `/token` 교환 함수 (PR #53)
    - 단계 3 ✅ 머지 (2026-05-07, PR #54): `state-machine.js`에 `_attemptSilentRefresh` + `acceptRedirectCode` 결합, `drive-sync.js` IIFE에 PKCE 콜백 흡수, MAX_REAUTH cap이 silent refresh 진입 전 적용되어 만성 401 무한 루프 차단. Cursor Bugbot 3차 race·URL leak·DISABLED 가드 정정 포함. 기존 GIS / Implicit 흐름은 그대로 살아 있어 사용자 영향 0 (IDB 빈 채면 폴백)
    - 단계 4 🟡 PR open (2026-05-07): GIS Token Client / Implicit Flow / FedCM 의존 모두 제거. 데스크탑·Android·iOS 동일하게 PKCE redirect 단일 경로. 상태 집합 축소 (`INITIALIZING`/`IDENTIFYING`/`AUTHENTICATING` 제거 → 6개로). `index.html`에서 `gsi/client` `<script>` + CSP의 `accounts.google.com` 모두 제거. `transport.js` GIS wrapper 7종 + Implicit Flow 함수 모두 제거, PKCE 함수는 canonical 이름 인계 (`beginRedirectAuthPKCE` → `beginRedirectAuth` 등). `drive-sync.signIn()`은 이제 머신 dispatch 없이 `beginRedirectAuth` 직접 호출 (DISABLED → silent refresh fire 직후 race window 회피). 기존 사용자는 cold start 1회 NEEDS_CONSENT 통과 후 재인증 (데이터 손실 0). **Cloud Console redirect URI 등록(`http://localhost:8080/`, `https://bible.anglican.kr/`) 배포 직전 필수**
    - 단계 5: silent-blocked 키 cleanup + pitfalls 문서 + 보안 감사 마무리 (예정)
    - 살아있는 설계 문서: `docs/design/pkce-migration.md`
  - 보안 감사 완료 (`docs/audit/2026-05-02-171111.md`): Critical 0, Medium 2건 수정
  - 미결: Google OAuth 앱 검수 통과 (제출 완료, 심사 대기), Cloud Console redirect URI 등록 (Phase 2f 배포 전 필수)
...
- 완료: 클라이언트 JS 유닛 테스트 (ADR-013, 2026-05-06~07)
  - `tests/unit/state-machine.test.js` (26, Phase 2h 단계 4 갱신) + `refresh-store.test.js` (13, Phase 2h 단계 1) + `transport-pkce.test.js` (23, Phase 2h 단계 2). Node 24 `--test` + 자체 vm 하네스, 62 케이스 통과
