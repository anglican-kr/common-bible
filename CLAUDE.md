...
.github/
  workflows/
    test.yml            ← CI: Node 24 + node --test (유닛, ADR-013)
docs/
...
테스트마다 `loadMachine()`이 새 vm 컨텍스트에 `js/sync/state-machine.js`를 로드하므로 클로저 상태가 케이스 간에 새지 않는다.
...
- 완료: Google Drive 자동 동기화 — Phase 2a~2f (ADR-011)
  ...
  - 진행 중 — **Phase 2h (2026-05-06~)**: Implicit Flow → Authorization Code + PKCE + refresh token 마이그레이션. ADR-011 원래 결정으로 회귀해 desktop·iOS 단일 경로 통일
    ...
    - 단계 3 🟡 PR open: `state-machine.js` silent refresh + `acceptRedirectCode`, `drive-sync.js` PKCE callback 흡수
    - 단계 4~5: GIS 제거 + 일원화 → 정리 (예정)
...
- 완료: 클라이언트 JS 유닛 테스트 (ADR-013, 2026-05-06)
  - `tests/unit/state-machine.test.js` 30+ 케이스, Node 24 `--test` + 자체 vm 하네스
  - CI: `.github/workflows/test.yml` `unit` job
