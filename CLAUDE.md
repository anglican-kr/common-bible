...
테스트마다 `loadMachine()`이 새 vm 컨텍스트에 `js/sync/state-machine.js`를 로드하므로 클로저 상태가 케이스 간에 새지 않는다. Phase 2i 시점 78 케이스(state-machine 42 + refresh-store 13 + transport-pkce 23) 통과 — Phase 2h 단계 4의 GIS 분기 제거에 더해 Phase 2i의 sync 캐시 시나리오 13건(Group 10 캐시 핵심 8건 + Group 11 스토리지 제약 회복 3건 + Group 12 세션 종료 시 캐시 클리어 2건)이 추가됨.
...
- 완료: 클라이언트 JS 유닛 테스트 (ADR-013, 2026-05-06~08)
  - `tests/unit/state-machine.test.js` (42, Phase 2i 캐시 핵심 8건 + 스토리지 제약 회복 3건 + 세션 종료 캐시 클리어 2건) + `refresh-store.test.js` (13, Phase 2h 단계 1) + `transport-pkce.test.js` (23, Phase 2h 단계 2). Node 24 `--test` + 자체 vm 하네스, 78 케이스 통과
