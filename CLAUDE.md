...
- 완료: Google Drive 자동 동기화 — Phase 2a~2f (ADR-011)
  ...
  - **Phase 2h (2026-05-06~07)**: Implicit Flow → Authorization Code + PKCE + refresh token 마이그레이션. ADR-011 원래 결정으로 회귀해 desktop·Android·iOS 단일 경로 통일
    - 단계 1 ✅ 머지: `js/sync/refresh-store.js` AES-GCM 암호화 IndexedDB (PR #52)
    - 단계 2 ✅ 머지: `transport.js` PKCE 유틸 + `/token` 교환 함수 (PR #53)
    - 단계 3 ✅ 머지 (PR #54): `state-machine.js` `_attemptSilentRefresh` + `acceptRedirectCode`, IIFE PKCE 콜백 흡수, MAX_REAUTH cap이 silent refresh 진입 전 적용. Bugbot 3차 race·URL leak·DISABLED 가드 정정. 기존 GIS / Implicit 흐름은 그대로 공존 (IDB 빈 채면 폴백)
    - 단계 4 ✅ 머지 (PR #57): GIS Token Client / Implicit Flow / FedCM 의존 모두 제거. 데스크탑·Android·iOS 동일하게 PKCE redirect 단일 경로. 상태 집합 축소 (`INITIALIZING`/`IDENTIFYING`/`AUTHENTICATING` 제거 → 6개). `index.html`에서 `gsi/client` `<script>` + CSP의 `accounts.google.com` 모두 제거. `transport.js` GIS wrapper 7종 + Implicit Flow 함수 제거, PKCE 함수 canonical 이름 인계. `drive-sync.signIn()`은 머신 dispatch 없이 `beginRedirectAuth` 직접 호출. Bugbot PR #57에서 IDB await 갭 race 가드 정정 (commit `f090c83`)
    - 단계 5 🟡 PR open (2026-05-07): `bible-drive-silent-blocked` localStorage one-shot cleanup, `coding-pitfalls.md` PKCE 함정 섹션 3개(§11~13), `docs/audit/2026-05-07-pkce-refresh-token.md` 보안 감사(Critical/High/Medium 0건), README.md PKCE 단일 경로로 갱신
  ...
  - 미결: Google OAuth 앱 검수 통과 (제출 완료, 심사 대기), Cloud Console redirect URI 등록 — `http://localhost:8080/`, `https://bible.anglican.kr/` (배포 전 필수)
